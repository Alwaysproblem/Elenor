builtin.module {
  tile.program @matmul_gather_add_tile(
      %task : !nest.task,
  ...
  nest.context @matmul_gather_add_context(
      %lhs : !nest.global_memref<128x64xbf16>,
    ...
    %indices_global = nest.subview %indices
        offsets = [0] sizes = [1024] strides = [1]
        : !nest.global_view<1024xi32>
    %output_global = nest.subview %output
        offsets = [0, 0, 0] sizes = [1, 128, 128] strides = [1, 1, 1]
        : !nest.global_view<1x128x128xbf16>

    %lhs_buffer = nest.alloc slot = "mga_lhs" role = "in"
        shape = [128, 64] dtype = "bf16" alignment = 256
        : !nest.l2_buffer<128x64xbf16>
    %rhs_buffer = nest.alloc slot = "mga_rhs" role = "in"
        shape = [64, 128] dtype = "bf16" alignment = 256
        : !nest.l2_buffer<64x128xbf16>
    %indices_buffer = nest.alloc slot = "mga_indices" role = "in"
        shape = [1024] dtype = "i32" alignment = 256
        : !nest.l2_buffer<1024xi32>
    %output_buffer = nest.alloc slot = "mga_output" role = "out"
  ...
  nexus.program @run_matmul_gather_add(
      %lhs : !nest.global_memref<128x64xbf16>,
      %rhs : !nest.global_memref<64x128xbf16>,
      %table : !nest.global_memref<8388608xi8>,
      %indices : !nest.global_memref<1024xi32>,
    ...
    %done = nexus.submit_context.async
        @matmul_gather_add_context(%lhs, %rhs, %table, %indices, %output)
        : !nexus.event<"matmul_gather_add_done">
    nexus.await %done
    nexus.return
  }
}
