builtin.module {
  tile.program @matmul_gather_add_4tile(
      %task : !nest.task,
  ...
  nest.context @matmul_gather_add_ctx0(
    ...
    %rhs_buffer = nest.alloc slot = "mga_ctx0_rhs" role = "in"
        shape = [4, 64, 128] dtype = "bf16" alignment = 256
        : !nest.l2_buffer<4x64x128xbf16>
    %indices_buffer = nest.alloc slot = "mga_ctx0_indices" role = "in"
    ...
    %grid_done, %input_released, %output_ready =
        nest.dispatch.tasks.async @matmul_gather_add_4tile context = 0
  ...
  nest.context @matmul_gather_add_ctx1(
      %lhs : !nest.global_memref<4x128x64xbf16>,
      %rhs : !nest.global_memref<4x64x128xbf16>,
      %table : !nest.global_memref<8388608xi8>,
      %indices : !nest.global_memref<4x16xi32>,
    ...
    %lhs_buffer = nest.alloc slot = "mga_ctx1_lhs" role = "in"
        shape = [4, 128, 64] dtype = "bf16" alignment = 256
        : !nest.l2_buffer<4x128x64xbf16>
    %rhs_buffer = nest.alloc slot = "mga_ctx1_rhs" role = "in"
        shape = [4, 64, 128] dtype = "bf16" alignment = 256
        : !nest.l2_buffer<4x64x128xbf16>
    %indices_buffer = nest.alloc slot = "mga_ctx1_indices" role = "in"
        shape = [4, 16] dtype = "i32" alignment = 256
        : !nest.l2_buffer<4x16xi32>
  ...
  nexus.program @run_matmul_gather_add_4tiles_2contexts(
      %table : !nest.global_memref<8388608xi8>,
