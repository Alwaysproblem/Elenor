builtin.module {
  tile.program @gather_matmul_tile(
      %task : !nest.task,
  ...
  nest.context @gather_matmul_context(
    ...
    %rhs_buffer = nest.alloc slot = "gm_rhs" role = "in"
        shape = [64, 128] dtype = "bf16" alignment = 256
        : !nest.l2_buffer<64x128xbf16>
    %indices_buffer = nest.alloc slot = "gm_indices" role = "in"
    ...
    %grid_done, %input_released, %output_ready =
        nest.dispatch.tasks.async @gather_matmul_tile
  ...
  nexus.program @run_gather_matmul(
      %lhs : !nest.global_memref<128x64xbf16>,
      %rhs : !nest.global_memref<64x128xbf16>,
      %table : !nest.global_memref<8388608xi8>,
      %indices : !nest.global_memref<1024xi32>,
