...
//         "W_P4": {
//           "index": 20,
//           "offset_elements": 2621440,
//           "bytes": 32768
//         },
//         "P4": {
//           "index": 21,
//           "offset_elements": 2752512,
//           "bytes": 32768
//         },
//         "R2": {
//           "index": 22,
//           "offset_elements": 2883584,
//           "bytes": 32768
//         }
//       },
//       "node_programs": {
...
//         "P4": ["input_P4.prefetch", "W_P4.prefetch"],
//         "R0": [
...
//         ],
//         "R": [
//           "R2.context_done 后从同一 HBM 区间 prefetch",
//           "P4.context_done 后从同一 HBM 区间 prefetch"
//         ],
...
//       },
//       "说明": "5 个 participant；P4 作为奇数尾项直达末层，R0/R1→R2 后与 P4→R，最终 Copy fanout 到 N0..N3。"
//     }
builtin.module {
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_input_P4 = nest.alloc slot = "input_P4" role = "in" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_input_P4 = nest.subview %arena offsets = [2490368] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_W_P4 = nest.alloc slot = "W_P4" role = "in" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_W_P4 = nest.subview %arena offsets = [2621440] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_P4 = nest.alloc slot = "P4" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_P4 = nest.subview %arena offsets = [2752512] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %pref_input_P4 = nest.dma.prefetch.async %h_input_P4 into %b_input_P4
      : !nest.event<"pref_input_P4">
    %pref_W_P4 = nest.dma.prefetch.async %h_W_P4 into %b_W_P4 : !nest.event<"pref_W_P4">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_P4, %read_P4, %ready_P4 = nest.dispatch.tasks.async @prog_P4 tasks(%tasks) globals()
      bindings(%b_input_P4, %b_W_P4, %b_P4) ins(%b_input_P4, %b_W_P4) outs(%b_P4)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_input_P4, %pref_W_P4)
      : (!nest.event<"grid_P4">, !nest.event<"read_P4">, !nest.event<"ready_P4">)
    nest.release %b_input_P4 depends_on(%read_P4, %pref_input_P4)
    nest.release %b_W_P4 depends_on(%read_P4, %pref_W_P4)
    %store_P4_0 = nest.dma.store.async %b_P4 into %h_P4 depends_on(%ready_P4)
      : !nest.event<"store_P4_0">
  ...
  nest.context @ctx_7 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_R1 = nest.subview %arena offsets = [1703936] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_R2 = nest.alloc slot = "R2" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_R2 = nest.subview %arena offsets = [2883584] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_R2, %read_R2, %ready_R2 = nest.dispatch.tasks.async @prog_R2 tasks(%tasks) globals()
      bindings(%b_R0, %b_R1, %b_R2) ins(%b_R0, %b_R1) outs(%b_R2)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_R0, %pref_R1)
      : (!nest.event<"grid_R2">, !nest.event<"read_R2">, !nest.event<"ready_R2">)
    nest.release %b_R0 depends_on(%read_R2, %pref_R0)
    nest.release %b_R1 depends_on(%read_R2, %pref_R1)
    %store_R2_0 = nest.dma.store.async %b_R2 into %h_R2 depends_on(%ready_R2)
      : !nest.event<"store_R2_0">
  ...
  nest.context @ctx_8 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_R2 = nest.alloc slot = "R2" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_R2 = nest.subview %arena offsets = [2883584] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_P4 = nest.alloc slot = "P4" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_P4 = nest.subview %arena offsets = [2752512] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %grid_R, %read_R, %ready_R = nest.dispatch.tasks.async @prog_R tasks(%tasks) globals()
      bindings(%b_R2, %b_P4, %b_R) ins(%b_R2, %b_P4) outs(%b_R)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_R2, %pref_P4)
  ...
  nexus.program @run (%arena: !nest.global_memref<4194304xbf16>) {
    ...
    nexus.await %done_0, %done_1, %done_2, %done_3, %done_4, %done_5, %done_6, %done_7, %done_8,
      %done_9, %done_10, %done_11, %done_12
    nexus.return
  }
}
