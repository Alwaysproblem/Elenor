...
builtin.module {
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_P4, %read_P4, %ready_P4 = nest.dispatch.tasks.async @prog_P4 tasks(%tasks) globals() bindings(%b_input_P4, %b_W_P4, %b_P4) ins(%b_input_P4, %b_W_P4) outs(%b_P4) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_P4, %pref_W_P4) : (!nest.event<"grid_P4">, !nest.event<"read_P4">, !nest.event<"ready_P4">)
    nest.release %b_input_P4 depends_on(%read_P4, %pref_input_P4)
    nest.release %b_W_P4 depends_on(%read_P4, %pref_W_P4)
  ...
  nest.context @ctx_7 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_R2, %read_R2, %ready_R2 = nest.dispatch.tasks.async @prog_R2 tasks(%tasks) globals() bindings(%b_R0, %b_R1, %b_R2) ins(%b_R0, %b_R1) outs(%b_R2) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_R0, %pref_R1) : (!nest.event<"grid_R2">, !nest.event<"read_R2">, !nest.event<"ready_R2">)
    nest.release %b_R0 depends_on(%read_R2, %pref_R0)
    nest.release %b_R1 depends_on(%read_R2, %pref_R1)
  ...
  nest.context @ctx_8 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_R, %read_R, %ready_R = nest.dispatch.tasks.async @prog_R tasks(%tasks) globals() bindings(%b_R2, %b_P4, %b_R) ins(%b_R2, %b_P4) outs(%b_R) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_R2, %pref_P4) : (!nest.event<"grid_R">, !nest.event<"read_R">, !nest.event<"ready_R">)
    nest.release %b_R2 depends_on(%read_R, %pref_R2)
    nest.release %b_P4 depends_on(%read_R, %pref_P4)
