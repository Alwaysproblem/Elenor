...
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 2 tasks(%tasks) globals() ins(%b_B, %b_C) outs(%b_B, %b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    ...
    %store_B_0 = nest.dma.store.async %b_B into %h_B depends_on(%ready_B, %ready_C) : !nest.event<"store_B_0">
