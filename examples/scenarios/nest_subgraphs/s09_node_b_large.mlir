...
//         ]
//       },
//       "说明": "仅 B output 扩为 4x256x64；C/D 的 B 输入 L1 和 transfer bytes 同步扩大，其他输出保持 U。"
//     }
builtin.module {
  tile.program @prog_A(
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_A = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_B = nest.alloc slot = "B" role = "inout" shape = [4, 256, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x256x64xbf16>
    %h_B = nest.subview %arena offsets = [393216] sizes = [65536] strides = [1]
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_A = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_B = nest.alloc slot = "B" role = "in" shape = [4, 256, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x256x64xbf16>
    %h_B = nest.subview %arena offsets = [393216] sizes = [65536] strides = [1]
      : !nest.global_view<65536xbf16>
