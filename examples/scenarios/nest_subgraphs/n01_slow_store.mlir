...
//       },
//       "forbidden_dependencies": "不得用 output_ready 代替 HBM 可见性，也不得用 device run 结束代替 tile_done",
//       "tensors": {
//         "Input": {
//           "index": 0,
//           "offset_elements": 0,
...
//           "bytes": 131072,
//           "arena_interval_bytes": [262144, 393216],
//           "role": "output"
//         }
//       },
//       "phase": {
//         "Compute": ["pref_Input"],
//         "context_done": ["grid_Compute", "store_Output_9"]
//       },
//       "pins": {
//         "Compute": 0
//       },
...
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_output = nest.subview %arena offsets = [131072] sizes = [65536] strides = [1]
    ...
    %grid_Compute, %read_Compute, %ready_Compute = nest.dispatch.tasks.async @prog_Compute
      ...
      : (!nest.event<"grid_Compute">, !nest.event<"read_Compute">, !nest.event<"ready_Compute">)
    nest.release %b_input depends_on(%read_Compute, %pref_input)
    %store_Output_0 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_0">
    nest.await %store_Output_0
    %store_Output_1 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_1">
    nest.await %store_Output_1
    %store_Output_2 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_2">
    nest.await %store_Output_2
    %store_Output_3 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_3">
    nest.await %store_Output_3
    %store_Output_4 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_4">
    nest.await %store_Output_4
    %store_Output_5 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_5">
    nest.await %store_Output_5
    %store_Output_6 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_6">
    nest.await %store_Output_6
    %store_Output_7 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_7">
    nest.await %store_Output_7
    %store_Output_8 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_8">
    nest.await %store_Output_8
    %store_Output_9 = nest.dma.store.async %b_output into %h_output depends_on(%ready_Compute)
      : !nest.event<"store_Output_9">
    nest.release %b_output depends_on(
      %store_Output_0, %store_Output_1, %store_Output_2, %store_Output_3, %store_Output_4,
      %store_Output_5, %store_Output_6, %store_Output_7, %store_Output_8, %store_Output_9)
