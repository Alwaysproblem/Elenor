...
//       "consumer_ready": {
//         "A": ["input_A.prefetch"],
//         "B": ["A.context_done 后从同一 HBM 区间 prefetch", "W_B.prefetch"],
//         "C": ["A.context_done 后从同一 HBM 区间 prefetch"],
//         "D": ["C.output_ready"],
//         "J": [
...
//         ]
//       },
//       "说明": "B 与 C→D 是无关分支；J 仍等待 B/D。reversed 只把 D 变慢，前三项只加长 B 分支的指定活动。"
//     }
builtin.module {
  tile.program @prog_A(
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_A, %read_A, %ready_A = nest.dispatch.tasks.async @prog_A context = 0 tasks(%tasks)
      globals() bindings(%b_input_A, %b_A) ins(%b_input_A) outs(%b_A)
      signal_policy {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_A = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_W_B = nest.alloc slot = "W_B" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_W_B = nest.subview %arena offsets = [131072] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B context = 1 tasks(%tasks)
      globals() bindings(%b_A, %b_W_B, %b_B) ins(%b_A, %b_W_B) outs(%b_B)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_A, %pref_W_B)
    ...
    nest.release %b_B depends_on(
      %store_B_0, %store_B_1, %store_B_2, %store_B_3, %store_B_4, %store_B_5, %store_B_6,
      %store_B_7, %store_B_8, %store_B_9)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 2 tasks(%tasks)
      ...
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_A) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D context = 2 tasks(%tasks)
      globals() bindings(%b_C, %b_D) ins(%b_C) outs(%b_D)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_C)
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_D = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_J = nest.alloc slot = "J" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_J = nest.subview %arena offsets = [786432] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_J, %read_J, %ready_J = nest.dispatch.tasks.async @prog_J context = 3 tasks(%tasks)
      globals() bindings(%b_B, %b_D, %b_J) ins(%b_B, %b_D) outs(%b_J)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_B, %pref_D)
      : (!nest.event<"grid_J">, !nest.event<"read_J">, !nest.event<"ready_J">)
