...
// case: {"id":"N04","name":"n04_fifo_hol","nodes":["Holder","Head","Tail"],"edges":{},"context_partition":[["Holder"],["Head"],["Tail"]],"resource_config":{"uce":4,"device_contexts":4,"fidelity":"full_memory","num_dma_channels":2,"arena_binding":"arena=0x1000000:8388608:rw","group_sram_bytes":196608,"placement":15},"expected":{"时序/生命周期 Correctness":"Holder 以 input2U+output4U 占满 6U；释放 input2U 时 Head 需要4U而不能接纳，Tail 虽只需2U也不得绕过；Holder output4U 释放后 admission 顺序 Head→Tail","数值":"未建模；engine descriptor 与未初始化的合成输出只用于时序","Liveness":"所有已接纳 Context 完成","Scheduling Quality":"仅记录实际 service/等待，不声称最优调度"},"forbidden_dependencies":"严格 FIFO 队头阻塞窗口禁止 Tail 绕过 Head；不得以不同 submit 时刻制造伪空闲","tensors":{"Holder_input":{"index":0,"offset_elements":0,"bytes":65536,"arena_interval_bytes":[0,65536],"role":"external_input"},"Head_input":{"index":1,"offset_elements":131072,"bytes":65536,"arena_interval_bytes":[262144,327680],"role":"external_input"},"Tail_input":{"index":2,"offset_elements":262144,"bytes":32768,"arena_interval_bytes":[524288,557056],"role":"external_input"},"Holder_output":{"index":3,"offset_elements":393216,"bytes":131072,"arena_interval_bytes":[786432,917504],"role":"output"},"Head_output":{"index":4,"offset_elements":524288,"bytes":65536,"arena_interval_bytes":[1048576,1114112],"role":"output"},"Tail_output":{"index":5,"offset_elements":655360,"bytes":32768,"arena_interval_bytes":[1310720,1343488],"role":"output"}},"phase":{"Holder":["pref_Holder_input"],"Head":["fifo_admission_after_Holder_output_release"],"Tail":["fifo_admission_after_Head"]},"pins":{"Holder":"device-slot","Head":"device-slot","Tail":"device-slot"},"node_programs":{"Holder":{"program":"prog_Holder","engine":"evu:expand","repeat":100,"pin":"device-slot"},"Head":{"program":"prog_Head","engine":"evu:relu","repeat":1,"pin":"device-slot"},"Tail":{"program":"prog_Tail","engine":"evu:relu","repeat":1,"pin":"device-slot"}}}
builtin.module {
  tile.program @prog_Holder (%task: !nest.task, %i0: !nest.l2_buffer<4x128x64xbf16>, %out: !nest.l2_buffer<4x256x64xbf16>) {
    %v0 = tile.subview %i0 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 128, 64] strides = [1, 1, 1] : !nest.l2_view<1x128x64xbf16>
  ...
  tile.program @prog_Head (%task: !nest.task, %i0: !nest.l2_buffer<4x128x64xbf16>, %out: !nest.l2_buffer<4x128x64xbf16>) {
    %v0 = tile.subview %i0 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 128, 64] strides = [1, 1, 1] : !nest.l2_view<1x128x64xbf16>
    %l0 = tile.alloc shape = [128, 64] dtype = "bf16" alignment = 256 : !tile.l1_buffer<128x64xbf16>
  ...
  tile.program @prog_Tail (%task: !nest.task, %i0: !nest.l2_buffer<4x64x64xbf16>, %out: !nest.l2_buffer<4x64x64xbf16>) {
  ...
  nest.context @ctx_holder (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_input = nest.alloc slot = "holder_input" role = "in" shape = [4, 128, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x128x64xbf16>
    %b_output = nest.alloc slot = "holder_output" role = "inout" shape = [4, 256, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x256x64xbf16>
    %h_input = nest.subview %arena offsets = [0] sizes = [32768] strides = [1] : !nest.global_view<32768xbf16>
    %h_output = nest.subview %arena offsets = [393216] sizes = [65536] strides = [1] : !nest.global_view<65536xbf16>
    %pref_input = nest.dma.prefetch.async %h_input into %b_input : !nest.event<"pref_Holder_input">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Holder, %read_Holder, %ready_Holder = nest.dispatch.tasks.async @prog_Holder tasks(%tasks) globals() ins(%b_input, %b_output) outs(%b_input, %b_output) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input) : (!nest.event<"grid_Holder">, !nest.event<"read_Holder">, !nest.event<"ready_Holder">)
    nest.release %b_input depends_on(%read_Holder)
    %store_Holder = nest.dma.store.async %b_output into %h_output depends_on(%ready_Holder) : !nest.event<"store_Holder">
  ...
  nest.context @ctx_head (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_input = nest.alloc slot = "head_input" role = "in" shape = [4, 128, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x128x64xbf16>
    %b_output = nest.alloc slot = "head_output" role = "inout" shape = [4, 128, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x128x64xbf16>
    %h_input = nest.subview %arena offsets = [131072] sizes = [32768] strides = [1] : !nest.global_view<32768xbf16>
    %h_output = nest.subview %arena offsets = [524288] sizes = [32768] strides = [1] : !nest.global_view<32768xbf16>
    %pref_input = nest.dma.prefetch.async %h_input into %b_input : !nest.event<"pref_Head_input">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Head, %read_Head, %ready_Head = nest.dispatch.tasks.async @prog_Head tasks(%tasks) globals() ins(%b_input, %b_output) outs(%b_input, %b_output) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input) : (!nest.event<"grid_Head">, !nest.event<"read_Head">, !nest.event<"ready_Head">)
    nest.release %b_input depends_on(%read_Head)
    %store_Head = nest.dma.store.async %b_output into %h_output depends_on(%ready_Head) : !nest.event<"store_Head">
  ...
  nest.context @ctx_tail (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_input = nest.alloc slot = "tail_input" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %b_output = nest.alloc slot = "tail_output" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_input = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    %h_output = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    %pref_input = nest.dma.prefetch.async %h_input into %b_input : !nest.event<"pref_Tail_input">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Tail, %read_Tail, %ready_Tail = nest.dispatch.tasks.async @prog_Tail tasks(%tasks) globals() ins(%b_input, %b_output) outs(%b_input, %b_output) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input) : (!nest.event<"grid_Tail">, !nest.event<"read_Tail">, !nest.event<"ready_Tail">)
    nest.release %b_input depends_on(%read_Tail)
    %store_Tail = nest.dma.store.async %b_output into %h_output depends_on(%ready_Tail) : !nest.event<"store_Tail">
