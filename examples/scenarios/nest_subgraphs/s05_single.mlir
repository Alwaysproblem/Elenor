...
// case: {"id": "s05", "name": "s05_single", "nodes": ["A", "B", "C", "D"], "edges": {"B": ["A"], "C": ["A"], "D": ["B", "C"]}, "context_partition": [["A", "B", "C", "D"]], "resource_config": {"uce": 4, "device": 1, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "以实际 service 判定；不把 issue/slot 占用冒充有效重叠"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_A": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_A": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "W_B": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "A": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "B": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "C": {"index": 5, "offset_elements": 655360, "bytes": 32768}, "D": {"index": 6, "offset_elements": 786432, "bytes": 32768}}, "node_programs": {"A": {"program": "prog_A", "pin": 0, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "B": {"program": "prog_B", "pin": 1, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "C": {"program": "prog_C", "pin": 2, "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "D": {"program": "prog_D", "pin": 3, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}}, "consumer_ready": {"A": ["input_A.prefetch", "W_A.prefetch"], "B": ["A.output_ready", "W_B.prefetch"], "C": ["A.output_ready"], "D": ["B.output_ready", "C.output_ready"]}}
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_A = nest.subview %arena offsets = [393216] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    %b_W_B = nest.alloc slot = "W_B" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_W_B = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    ...
    %h_C = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    %b_D = nest.alloc slot = "D" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_D = nest.subview %arena offsets = [786432] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    ...
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D context = 3 tasks(%tasks) globals() ins(%b_B, %b_C, %b_D) outs(%b_B, %b_C, %b_D) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_B, %ready_C) : (!nest.event<"grid_D">, !nest.event<"read_D">, !nest.event<"ready_D">)
    ...
    %store_B_0 = nest.dma.store.async %b_B into %h_B depends_on(%ready_B, %ready_D) : !nest.event<"store_B_0">
