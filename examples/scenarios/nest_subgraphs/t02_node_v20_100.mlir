...
//         "Scheduling Quality": "node 转置顺序优先右上推进；V20 重 compute100，检查无关 V02 是否推进"
//       },
...
//         "V00->V01": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V00->V10": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V01->V11": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V10->V11": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V10->V20": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V01->V02": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V11->V21": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V20->V21": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V02->V12": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V11->V12": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V12->V22": "producer context_done 后从同一 HBM 区间 prefetch",
//         "V21->V22": "producer context_done 后从同一 HBM 区间 prefetch"
//       },
//       "notes": {
...
builtin.module {
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    nest.release %b_V00 depends_on(%read_V01, %pref_V00)
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    nest.release %b_V01 depends_on(%read_V02, %pref_V01)
  ...
  nest.context @ctx_7 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_V11 = nest.subview %arena offsets = [524288] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_V20 = nest.alloc slot = "V20" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_V20 = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
  ...
  nest.context @ctx_8 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_V12 = nest.subview %arena offsets = [1048576] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_V21 = nest.alloc slot = "V21" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_V21 = nest.subview %arena offsets = [917504] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
