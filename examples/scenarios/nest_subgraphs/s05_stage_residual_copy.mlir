...
// case: {
//       "id": "s05",
//       "name": "s05_stage_residual_copy",
//       "nodes": ["X", "F0", "F1", "Copy", "Add"],
//       "edges": {
//         "F0": ["X"],
//         "F1": ["F0"],
//         "Copy": ["X"],
...
//       "consumer_ready": {
//         "X": ["input_X.prefetch"],
//         "F0": ["X.context_done 后从同一 HBM 区间 prefetch", "W_F0.prefetch"],
//         "F1": ["F0.output_ready"],
//         "Copy": ["X.context_done 后从同一 HBM 区间 prefetch"],
//         "Add": [
//           "Copy.context_done 后从同一 HBM 区间 prefetch",
//           "F1.context_done 后从同一 HBM 区间 prefetch"
//         ]
//       },
//       "说明": "Copy 真实 load X 并 Store 到独立 Skip/Copy arena 区间；Add 读取 Copy 和 F1。"
//     }
builtin.module {
  tile.program @prog_X(
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_X = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_W_F0 = nest.alloc slot = "W_F0" role = "in" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_W_F0 = nest.subview %arena offsets = [131072] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_F0 = nest.alloc slot = "F0" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_F0 = nest.subview %arena offsets = [393216] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_F1 = nest.alloc slot = "F1" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_F1 = nest.subview %arena offsets = [524288] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %grid_F1, %read_F1, %ready_F1 = nest.dispatch.tasks.async @prog_F1 context = 1 tasks(%tasks)
      ...
      : (!nest.event<"grid_F1">, !nest.event<"read_F1">, !nest.event<"ready_F1">)
    nest.release %b_X depends_on(%read_F0, %pref_X)
    nest.release %b_W_F0 depends_on(%read_F0, %pref_W_F0)
    %store_F0_0 = nest.dma.store.async %b_F0 into %h_F0 depends_on(%ready_F0)
      : !nest.event<"store_F0_0">
    nest.release %b_F0 depends_on(%read_F1, %store_F0_0)
    %store_F1_0 = nest.dma.store.async %b_F1 into %h_F1 depends_on(%ready_F1)
      : !nest.event<"store_F1_0">
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_X = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_Copy = nest.alloc slot = "Copy" role = "inout" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_Copy = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %pref_X = nest.dma.prefetch.async %h_X into %b_X : !nest.event<"pref_X">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Copy, %read_Copy, %ready_Copy = nest.dispatch.tasks.async @prog_Copy context = 2
      tasks(%tasks) globals() bindings(%b_X, %b_Copy) ins(%b_X) outs(%b_Copy)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_X)
      : (!nest.event<"grid_Copy">, !nest.event<"read_Copy">, !nest.event<"ready_Copy">)
    nest.release %b_X depends_on(%read_Copy, %pref_X)
    %store_Copy_0 = nest.dma.store.async %b_Copy into %h_Copy depends_on(%ready_Copy)
      : !nest.event<"store_Copy_0">
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_Copy = nest.alloc slot = "Copy" role = "in" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_Copy = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_F1 = nest.alloc slot = "F1" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_F1 = nest.subview %arena offsets = [524288] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_Add = nest.alloc slot = "Add" role = "inout" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_Add = nest.subview %arena offsets = [786432] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %grid_Add, %read_Add, %ready_Add = nest.dispatch.tasks.async @prog_Add context = 3 tasks(%tasks)
      globals() bindings(%b_Copy, %b_F1, %b_Add) ins(%b_Copy, %b_F1) outs(%b_Add)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_Copy, %pref_F1)
      : (!nest.event<"grid_Add">, !nest.event<"read_Add">, !nest.event<"ready_Add">)
    nest.release %b_Copy depends_on(%read_Add, %pref_Copy)
    nest.release %b_F1 depends_on(%read_Add, %pref_F1)
    %store_Add_0 = nest.dma.store.async %b_Add into %h_Add depends_on(%ready_Add)
      : !nest.event<"store_Add_0">
