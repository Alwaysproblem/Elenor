...
//         "A": ["input_A.prefetch"],
//         "B": ["A.output_ready"],
//         "C": ["A.output_ready", "W_C.prefetch"],
//         "D": ["B.output_ready"],
//         "E": ["B.output_ready"],
//         "F": ["D.output_ready", "E.output_ready"],
//         "G": ["F.output_ready", "C.output_ready"]
//       }
//     }
builtin.module {
  tile.program @prog_A(
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 2 tasks(%tasks)
      globals() bindings(%b_A, %b_W_C, %b_C) ins(%b_A, %b_W_C) outs(%b_C)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_A, %pref_W_C)
      : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D context = 0 tasks(%tasks)
    ...
    %grid_E, %read_E, %ready_E = nest.dispatch.tasks.async @prog_E context = 1 tasks(%tasks)
      globals() bindings(%b_B, %b_E) ins(%b_B) outs(%b_E)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_B)
      : (!nest.event<"grid_E">, !nest.event<"read_E">, !nest.event<"ready_E">)
    %grid_F, %read_F, %ready_F = nest.dispatch.tasks.async @prog_F context = 3 tasks(%tasks)
      globals() bindings(%b_D, %b_E, %b_F) ins(%b_D, %b_E) outs(%b_F)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_D, %ready_E)
      : (!nest.event<"grid_F">, !nest.event<"read_F">, !nest.event<"ready_F">)
    %grid_G, %read_G, %ready_G = nest.dispatch.tasks.async @prog_G context = 0 tasks(%tasks)
      globals() bindings(%b_F, %b_C, %b_G) ins(%b_F, %b_C) outs(%b_G)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_F, %ready_C)
    ...
    nest.await %grid_A, %grid_B, %grid_C, %grid_D, %grid_E, %grid_F, %grid_G, %store_A_0, %store_B_0
      , %store_C_0, %store_D_0, %store_E_0, %store_F_0, %store_G_0
    nest.return
  }
