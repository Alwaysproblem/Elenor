...
builtin.module {
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %pref_X = nest.dma.prefetch.async %h_X into %b_X : !nest.event<"pref_X">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_V, %read_V, %ready_V = nest.dispatch.tasks.async @prog_V tasks(%tasks) globals() bindings(%b_X, %b_V) ins(%b_X) outs(%b_V) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_X) : (!nest.event<"grid_V">, !nest.event<"read_V">, !nest.event<"ready_V">)
