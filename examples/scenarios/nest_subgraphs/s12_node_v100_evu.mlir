...
//       "consumer_ready": {
//         "X": ["input_X.prefetch"],
//         "Q": ["X.context_done 后从同一 HBM 区间 prefetch", "W_Q.prefetch"],
//         "K": ["X.context_done 后从同一 HBM 区间 prefetch", "W_K.prefetch"],
//         "V": ["X.context_done 后从同一 HBM 区间 prefetch"],
//         "QK": [
//           "Q.context_done 后从同一 HBM 区间 prefetch",
//           "K.context_done 后从同一 HBM 区间 prefetch"
//         ],
//         "SM": ["QK.context_done 后从同一 HBM 区间 prefetch"],
//         "PV": [
//           "SM.context_done 后从同一 HBM 区间 prefetch",
//           "V.context_done 后从同一 HBM 区间 prefetch"
//         ],
//         "O": ["PV.context_done 后从同一 HBM 区间 prefetch"]
//       },
//       "说明": "V 改为 EVU100，Q/K/QK 保持 BOA；W_V 的基础 arena index 保留为空洞，全部节点 output index 不变。QK 无 V 硬边。",
...
builtin.module {
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_V, %read_V, %ready_V = nest.dispatch.tasks.async @prog_V tasks(%tasks) globals()
      bindings(%b_X, %b_V) ins(%b_X) outs(%b_V)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_X) : (!nest.event<"grid_V">, !nest.event<"read_V">, !nest.event<"ready_V">)
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_Q = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_K = nest.alloc slot = "K" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_K = nest.subview %arena offsets = [786432] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
  ...
  nest.context @ctx_6 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_SM = nest.subview %arena offsets = [1179648] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %b_V = nest.alloc slot = "V" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_V = nest.subview %arena offsets = [917504] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
