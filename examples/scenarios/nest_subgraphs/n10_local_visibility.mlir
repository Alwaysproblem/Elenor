...
// case: {"id":"N10","name":"n10_local_visibility","nodes":["A","B"],"edges":{"B":["A"]},"context_partition":[["A","B"]],"resource_config":{"uce":4,"device_contexts":4,"fidelity":"full_memory","num_dma_channels":2,"arena_binding":"arena=0x1000000:8388608:rw","placement":15},"expected":{"时序/生命周期 Correctness":"同 Context 中 A global Store 完成事件被显式 nest.await；只有越过该 PC await 后才从同一 HBM 地址 prefetch 到独立 B_input L2","数值":"未建模；engine descriptor 与未初始化的合成输出只用于时序","Liveness":"所有已接纳 Context 完成","Scheduling Quality":"仅记录实际 service/等待，不声称最优调度"},"forbidden_dependencies":"禁止给 prefetch 捏造 depends_on；禁止直接把 A 的 output_ready 当作 HBM 可见","tensors":{"A_input":{"index":0,"offset_elements":0,"bytes":32768,"arena_interval_bytes":[0,32768],"role":"external_input"},"A_output/B_input":{"index":1,"offset_elements":131072,"bytes":32768,"arena_interval_bytes":[262144,294912],"role":"producer_output_and_consumer_input","alias":true},"B_output":{"index":2,"offset_elements":262144,"bytes":32768,"arena_interval_bytes":[524288,557056],"role":"output"}},"phase":{"A":["pref_A_input"],"B":["explicit nest.await store_done","pref_A_output_without_depends_on"]},"pins":{"A":0,"B":1},"node_programs":{"A":{"program":"prog_A","engine":"evu:relu","repeat":1,"pin":0},"B":{"program":"prog_B","engine":"evu:relu","repeat":1,"pin":1}}}
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_A, %read_A, %ready_A = nest.dispatch.tasks.async @prog_A context = 0 tasks(%tasks) globals() ins(%b_A_input, %b_A_output) outs(%b_A_input, %b_A_output) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A_input) : (!nest.event<"grid_A">, !nest.event<"read_A">, !nest.event<"ready_A">)
    nest.release %b_A_input depends_on(%read_A)
    %store_done = nest.dma.store.async %b_A_output into %h_A_output depends_on(%ready_A) : !nest.event<"store_done">
    ...
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B context = 1 tasks(%tasks) globals() ins(%b_B_input, %b_B_output) outs(%b_B_input, %b_B_output) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A_output) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
    nest.release %b_B_input depends_on(%read_B)
