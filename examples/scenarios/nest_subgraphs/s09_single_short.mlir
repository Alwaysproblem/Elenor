...
// case: {"id": "s09", "name": "s09_single_short", "nodes": ["A", "B", "C", "D"], "edges": {"B": ["A"], "C": ["A", "B"], "D": ["A", "B", "C"]}, "context_partition": [["A", "B", "C", "D"]], "resource_config": {"uce": 4, "device": 1, "placement": 15, "fidelity": "full_memory", "group_sram_bytes": 196352}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期运行期 L2 永久容量 fault，非成功完成", "Scheduling Quality": "以实际 service 判定；不把 issue/slot 占用冒充有效重叠"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_A": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_A": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "A": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "B": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "C": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "D": {"index": 5, "offset_elements": 655360, "bytes": 32768}}, "node_programs": {"A": {"program": "prog_A", "pin": 0, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "B": {"program": "prog_B", "pin": 1, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}, "C": {"program": "prog_C", "pin": 2, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}, "D": {"program": "prog_D", "pin": 3, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}}, "consumer_ready": {"A": ["input_A.prefetch", "W_A.prefetch"], "B": ["A.output_ready"], "C": ["A.output_ready", "B.output_ready"], "D": ["A.output_ready", "B.output_ready", "C.output_ready"]}, "说明": "完整 bundle 为 6U=196608 B；配置容量 196352 B。short 应为 admission capacity fault，不是等待。"}
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 2 tasks(%tasks) globals() ins(%b_A, %b_B, %b_C) outs(%b_A, %b_B, %b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A, %ready_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D context = 3 tasks(%tasks) globals() ins(%b_A, %b_B, %b_C, %b_D) outs(%b_A, %b_B, %b_C, %b_D) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A, %ready_B, %ready_C) : (!nest.event<"grid_D">, !nest.event<"read_D">, !nest.event<"ready_D">)
    nest.release %b_input_A depends_on(%read_A)
    nest.release %b_W_A depends_on(%read_A)
    %store_A_0 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A, %ready_B, %ready_C, %ready_D) : !nest.event<"store_A_0">
    nest.release %b_A depends_on(%store_A_0)
    %store_B_0 = nest.dma.store.async %b_B into %h_B depends_on(%ready_B, %ready_C, %ready_D) : !nest.event<"store_B_0">
