...
//       },
//       "expected": {
//         "时序/生命周期 Correctness": "placement=15 的 popcount=4，但 task.range 仅为0..3；除此之外 program/signature/view/actual/release/store 均合法",
...
//       },
//       "phase": {
//         "InvalidCount": ["pref_Input"],
//         "invalid_invariant": "placement=15 的 popcount=4，但 task.range 仅为0..3"
//       },
//       "pins": {
//         "InvalidCount": "device-slot"
//       },
//       "node_programs": {
//         "InvalidCount": {
//           "program": "prog_InvalidCount",
//           "engine": "evu:relu",
//           "repeat": 1,
//           "pin": "device-slot"
//         }
...
//     }
builtin.module {
  tile.program @prog_InvalidCount(
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_InvalidCount, %read_InvalidCount, %ready_InvalidCount =
      nest.dispatch.tasks.async @prog_InvalidCount tasks(%tasks) globals()
      bindings(%b_input, %b_output) ins(%b_input) outs(%b_output)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_input) : (
        !nest.event<"grid_InvalidCount">, !nest.event<"read_InvalidCount">,
        !nest.event<"ready_InvalidCount">)
    nest.release %b_input depends_on(%read_InvalidCount, %pref_input)
    %store_Output = nest.dma.store.async %b_output into %h_output depends_on(%ready_InvalidCount)
