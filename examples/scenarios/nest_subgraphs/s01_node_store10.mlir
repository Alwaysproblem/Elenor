...
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %store_A_1 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_1">
    nest.await %store_A_1
    %store_A_2 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_2">
    nest.await %store_A_2
    %store_A_3 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_3">
    nest.await %store_A_3
    %store_A_4 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_4">
    nest.await %store_A_4
    %store_A_5 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_5">
    nest.await %store_A_5
    %store_A_6 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_6">
    nest.await %store_A_6
    %store_A_7 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_7">
    nest.await %store_A_7
    %store_A_8 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_8">
    nest.await %store_A_8
    %store_A_9 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A) : !nest.event<"store_A_9">
    nest.release %b_A depends_on(%store_A_0, %store_A_1, %store_A_2, %store_A_3, %store_A_4, %store_A_5, %store_A_6, %store_A_7, %store_A_8, %store_A_9)
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B tasks(%tasks) globals() bindings(%b_A, %b_Bias, %b_B) ins(%b_A, %b_Bias) outs(%b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A, %pref_Bias) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C tasks(%tasks) globals() bindings(%b_B, %b_C) ins(%b_B) outs(%b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D tasks(%tasks) globals() bindings(%b_C, %b_W_D, %b_D) ins(%b_C, %b_W_D) outs(%b_D) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_C, %pref_W_D) : (!nest.event<"grid_D">, !nest.event<"read_D">, !nest.event<"ready_D">)
