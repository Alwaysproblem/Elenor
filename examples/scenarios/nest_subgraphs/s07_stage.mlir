...
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B context = 1 tasks(%tasks) globals() bindings(%b_A, %b_B) ins(%b_A) outs(%b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_G, %read_G, %ready_G = nest.dispatch.tasks.async @prog_G context = 0 tasks(%tasks) globals() bindings(%b_F, %b_C, %b_G) ins(%b_F, %b_C) outs(%b_G) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_F, %pref_C) : (!nest.event<"grid_G">, !nest.event<"read_G">, !nest.event<"ready_G">)
