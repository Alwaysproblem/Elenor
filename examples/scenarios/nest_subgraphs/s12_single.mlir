...
// case: {"id": "s12", "name": "s12_single", "nodes": ["X", "Q", "K", "V", "QK", "SM", "PV", "O"], "edges": {"Q": ["X"], "K": ["X"], "V": ["X"], "QK": ["Q", "K"], "SM": ["QK"], "PV": ["SM", "V"], "O": ["PV"]}, "context_partition": [["X", "Q", "K", "V", "QK", "SM", "PV", "O"]], "resource_config": {"uce": 4, "device": 1, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "以实际 service 判定；不把 issue/slot 占用冒充有效重叠"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_X": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_Q": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "W_K": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "W_V": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "X": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "Q": {"index": 5, "offset_elements": 655360, "bytes": 32768}, "K": {"index": 6, "offset_elements": 786432, "bytes": 32768}, "V": {"index": 7, "offset_elements": 917504, "bytes": 32768}, "QK": {"index": 8, "offset_elements": 1048576, "bytes": 32768}, "SM": {"index": 9, "offset_elements": 1179648, "bytes": 32768}, "PV": {"index": 10, "offset_elements": 1310720, "bytes": 32768}, "O": {"index": 11, "offset_elements": 1441792, "bytes": 32768}}, "node_programs": {"X": {"program": "prog_X", "pin": 0, "engine": "copy", "op": "copy", "repeat": 1, "loads": 1}, "Q": {"program": "prog_Q", "pin": 0, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "K": {"program": "prog_K", "pin": 1, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "V": {"program": "prog_V", "pin": 2, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "QK": {"program": "prog_QK", "pin": 0, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "SM": {"program": "prog_SM", "pin": 1, "engine": "evu", "op": "softmax", "repeat": 1, "loads": 1}, "PV": {"program": "prog_PV", "pin": 0, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "O": {"program": "prog_O", "pin": 0, "engine": "copy", "op": "copy", "repeat": 1, "loads": 1}}, "consumer_ready": {"X": ["input_X.prefetch"], "Q": ["X.output_ready", "W_Q.prefetch"], "K": ["X.output_ready", "W_K.prefetch"], "V": ["X.output_ready", "W_V.prefetch"], "QK": ["Q.output_ready", "K.output_ready"], "SM": ["QK.output_ready"], "PV": ["SM.output_ready", "V.output_ready"], "O": ["PV.output_ready"]}}
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_X, %read_X, %ready_X = nest.dispatch.tasks.async @prog_X context = 0 tasks(%tasks) globals() ins(%b_input_X, %b_X) outs(%b_input_X, %b_X) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_X) : (!nest.event<"grid_X">, !nest.event<"read_X">, !nest.event<"ready_X">)
    %grid_Q, %read_Q, %ready_Q = nest.dispatch.tasks.async @prog_Q context = 0 tasks(%tasks) globals() ins(%b_X, %b_W_Q, %b_Q) outs(%b_X, %b_W_Q, %b_Q) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_X, %pref_W_Q) : (!nest.event<"grid_Q">, !nest.event<"read_Q">, !nest.event<"ready_Q">)
    %grid_K, %read_K, %ready_K = nest.dispatch.tasks.async @prog_K context = 1 tasks(%tasks) globals() ins(%b_X, %b_W_K, %b_K) outs(%b_X, %b_W_K, %b_K) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_X, %pref_W_K) : (!nest.event<"grid_K">, !nest.event<"read_K">, !nest.event<"ready_K">)
    %grid_V, %read_V, %ready_V = nest.dispatch.tasks.async @prog_V context = 2 tasks(%tasks) globals() ins(%b_X, %b_W_V, %b_V) outs(%b_X, %b_W_V, %b_V) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_X, %pref_W_V) : (!nest.event<"grid_V">, !nest.event<"read_V">, !nest.event<"ready_V">)
    %grid_QK, %read_QK, %ready_QK = nest.dispatch.tasks.async @prog_QK context = 0 tasks(%tasks) globals() ins(%b_Q, %b_K, %b_QK) outs(%b_Q, %b_K, %b_QK) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_Q, %ready_K) : (!nest.event<"grid_QK">, !nest.event<"read_QK">, !nest.event<"ready_QK">)
    %grid_SM, %read_SM, %ready_SM = nest.dispatch.tasks.async @prog_SM context = 1 tasks(%tasks) globals() ins(%b_QK, %b_SM) outs(%b_QK, %b_SM) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_QK) : (!nest.event<"grid_SM">, !nest.event<"read_SM">, !nest.event<"ready_SM">)
    %grid_PV, %read_PV, %ready_PV = nest.dispatch.tasks.async @prog_PV context = 0 tasks(%tasks) globals() ins(%b_SM, %b_V, %b_PV) outs(%b_SM, %b_V, %b_PV) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_SM, %ready_V) : (!nest.event<"grid_PV">, !nest.event<"read_PV">, !nest.event<"ready_PV">)
    %grid_O, %read_O, %ready_O = nest.dispatch.tasks.async @prog_O context = 0 tasks(%tasks) globals() ins(%b_PV, %b_O) outs(%b_PV, %b_O) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_PV) : (!nest.event<"grid_O">, !nest.event<"read_O">, !nest.event<"ready_O">)
    ...
    %store_X_0 = nest.dma.store.async %b_X into %h_X depends_on(%ready_X, %ready_Q, %ready_K, %ready_V) : !nest.event<"store_X_0">
    nest.release %b_X depends_on(%store_X_0)
    %store_Q_0 = nest.dma.store.async %b_Q into %h_Q depends_on(%ready_Q, %ready_QK) : !nest.event<"store_Q_0">
    nest.release %b_Q depends_on(%store_Q_0)
    %store_K_0 = nest.dma.store.async %b_K into %h_K depends_on(%ready_K, %ready_QK) : !nest.event<"store_K_0">
    nest.release %b_K depends_on(%store_K_0)
    %store_V_0 = nest.dma.store.async %b_V into %h_V depends_on(%ready_V, %ready_PV) : !nest.event<"store_V_0">
    nest.release %b_V depends_on(%store_V_0)
    %store_QK_0 = nest.dma.store.async %b_QK into %h_QK depends_on(%ready_QK, %ready_SM) : !nest.event<"store_QK_0">
    nest.release %b_QK depends_on(%store_QK_0)
    %store_SM_0 = nest.dma.store.async %b_SM into %h_SM depends_on(%ready_SM, %ready_PV) : !nest.event<"store_SM_0">
    nest.release %b_SM depends_on(%store_SM_0)
    %store_PV_0 = nest.dma.store.async %b_PV into %h_PV depends_on(%ready_PV, %ready_O) : !nest.event<"store_PV_0">
    ...
    nest.await %grid_X, %grid_Q, %grid_K, %grid_V, %grid_QK, %grid_SM, %grid_PV, %grid_O, %store_X_0, %store_Q_0, %store_K_0, %store_V_0, %store_QK_0, %store_SM_0, %store_PV_0, %store_O_0
    nest.return
  }
