...
//       },
//       "forbidden_dependencies": "禁止把永久不足排入 admission wait；禁止部分分配后启动 DMA/engine",
//       "tensors": {
//         "Input": {
//           "index": 0,
//           "offset_elements": 0,
...
//       },
//       "phase": {
//         "Impossible": ["context_admission_only"],
...
builtin.module {
  tile.program @prog_Impossible(
    ...
    %lout = tile.alloc shape = [128, 64] dtype = "bf16" alignment = 256
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_output = nest.subview %arena offsets = [131072] sizes = [32768] strides = [1]
    ...
    %grid_Impossible, %read_Impossible, %ready_Impossible =
      ...
      signal_policy {
        ...
        !nest.event<"grid_Impossible">, !nest.event<"read_Impossible">,
        !nest.event<"ready_Impossible">)
    nest.release %b_input depends_on(%read_Impossible, %pref_input)
    %store_Output = nest.dma.store.async %b_output into %h_output depends_on(%ready_Impossible)
