...
//         "Liveness": "预期完成",
//         "Scheduling Quality": "C 受当前跨 Context 完成事件粒度影响，还会等待含 D 的 B/D Context；这是刻意记录的非最优 mapping"
//       },
...
//         ]
//       },
//       "说明": "A/B 为独立根；正例发射 A、B、D、C，D 只依赖 B，C 实读 A/B。"
//     }
builtin.module {
  tile.program @prog_A(
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %b_A = nest.alloc slot = "A" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_A = nest.subview %arena offsets = [393216] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %b_B = nest.alloc slot = "B" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x64x64xbf16>
    %h_B = nest.subview %arena offsets = [524288] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D context = 2 tasks(%tasks)
      globals() bindings(%b_B, %b_D) ins(%b_B) outs(%b_D)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_B)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 3 tasks(%tasks)
      globals() bindings(%b_A, %b_B, %b_C) ins(%b_A, %b_B) outs(%b_C)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_A, %pref_B)
