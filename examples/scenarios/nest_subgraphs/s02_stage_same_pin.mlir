...
//         "C2": ["C1.output_ready"]
//       },
//       "说明": "所有 dispatch 固定 pin=0 以制造争用；物理 resource_config 仍为 4×4，不冒充 1 UCE。"
//     }
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_B0, %read_B0, %ready_B0 = nest.dispatch.tasks.async @prog_B0 context = 0 tasks(%tasks)
    ...
    %grid_B1, %read_B1, %ready_B1 = nest.dispatch.tasks.async @prog_B1 context = 0 tasks(%tasks)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_C0, %read_C0, %ready_C0 = nest.dispatch.tasks.async @prog_C0 context = 0 tasks(%tasks)
    ...
    %grid_C1, %read_C1, %ready_C1 = nest.dispatch.tasks.async @prog_C1 context = 0 tasks(%tasks)
    ...
    %grid_C2, %read_C2, %ready_C2 = nest.dispatch.tasks.async @prog_C2 context = 0 tasks(%tasks)
