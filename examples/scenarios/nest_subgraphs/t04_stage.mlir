...
builtin.module {
  ...
  nest.context @ctx_request_slow (%Req: !nest.global_memref<4194304xbf16>, %W: !nest.global_memref<4x64x64xbf16>) placement = 15 {
    %b_input = nest.alloc slot = "input" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %b_W = nest.alloc slot = "W" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    ...
    %grid_A, %read_A, %ready_A = nest.dispatch.tasks.async @prog_A_slow tasks(%tasks) globals() ins(%b_input, %b_W, %b_A) outs(%b_input, %b_W, %b_A) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input, %pref_W) : (!nest.event<"grid_A">, !nest.event<"read_A">, !nest.event<"ready_A">)
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B tasks(%tasks) globals() ins(%b_A, %b_B) outs(%b_A, %b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C tasks(%tasks) globals() ins(%b_B, %b_C) outs(%b_B, %b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    nest.release %b_input depends_on(%read_A)
    nest.release %b_W depends_on(%read_A)
    %store_A = nest.dma.store.async %b_A into %h_A depends_on(%ready_A, %ready_B) : !nest.event<"store_A">
    nest.release %b_A depends_on(%store_A)
    %store_B = nest.dma.store.async %b_B into %h_B depends_on(%ready_B, %ready_C) : !nest.event<"store_B">
  ...
  nest.context @ctx_request_fast (%Req: !nest.global_memref<4194304xbf16>, %W: !nest.global_memref<4x64x64xbf16>) placement = 15 {
    %b_input = nest.alloc slot = "input" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %b_W = nest.alloc slot = "W" role = "in" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %b_A = nest.alloc slot = "A" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %b_B = nest.alloc slot = "B" role = "inout" shape = [4, 64, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    ...
    %pref_W = nest.dma.prefetch.async %h_W into %b_W : !nest.event<"pref_W">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_A, %read_A, %ready_A = nest.dispatch.tasks.async @prog_A_fast tasks(%tasks) globals() ins(%b_input, %b_W, %b_A) outs(%b_input, %b_W, %b_A) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input, %pref_W) : (!nest.event<"grid_A">, !nest.event<"read_A">, !nest.event<"ready_A">)
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B tasks(%tasks) globals() ins(%b_A, %b_B) outs(%b_A, %b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C tasks(%tasks) globals() ins(%b_B, %b_C) outs(%b_B, %b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    nest.release %b_input depends_on(%read_A)
    nest.release %b_W depends_on(%read_A)
  ...
  nexus.program @run (%R0: !nest.global_memref<4194304xbf16>, %R1: !nest.global_memref<4194304xbf16>, %R2: !nest.global_memref<4194304xbf16>, %R3: !nest.global_memref<4194304xbf16>, %W: !nest.global_memref<4x64x64xbf16>) {
    %done_R0 = nexus.submit_context.async @ctx_request_slow(%R0, %W) : !nexus.event<"done_R0">
