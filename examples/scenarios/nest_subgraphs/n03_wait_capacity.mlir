...
//         "Waiter": ["admission_after_release_Holder_input", "pref_Waiter_input"]
//       },
//       "pins": {
//         "Holder": "device-slot",
//         "Waiter": "device-slot"
...
builtin.module {
  ...
  nest.context @ctx_holder (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %b_holder_output = nest.alloc slot = "holder_output" role = "inout" shape = [4, 64, 64]
      dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_holder_input = nest.subview %arena offsets = [0] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    %h_holder_output = nest.subview %arena offsets = [262144] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %store_Holder = nest.dma.store.async %b_holder_output into %h_holder_output
      depends_on(%ready_Holder) : !nest.event<"store_Holder">
  ...
  nest.context @ctx_waiter (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_Waiter, %read_Waiter, %unused_ready = nest.dispatch.tasks.async @prog_Waiter tasks(%tasks)
      ...
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
      } depends_on(%pref_waiter_input)
      : (!nest.event<"grid_Waiter">, !nest.event<"read_Waiter">, !nest.event<"">)
