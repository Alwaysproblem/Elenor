...
//         "Liveness": "预期完成",
//         "Scheduling Quality": "FAIL：刻意等待 A/B Context 后再连续 submit C、D；工作量和硬件与正例相同"
//       },
...
//         ]
//       },
//       "说明": "D 没有 C 依赖，也不等待 C Context 完成；反例只把 C 的 A/B 必要等待移到 D 提交之前。"
//     }
builtin.module {
  tile.program @prog_A(
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C tasks(%tasks) globals()
      bindings(%b_A, %b_B, %b_C) ins(%b_A, %b_B) outs(%b_C)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
