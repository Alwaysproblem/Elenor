...
//         ]
//       },
//       "说明": "A output 与 BOA 第一输入同步扩大为 4x256x64；ops=2097152，B/C/D 的 A 输入 L1 和 transfer bytes 同步扩大。"
//     }
builtin.module {
  tile.program @prog_A(
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_input_A = nest.alloc slot = "input_A" role = "in" shape = [4, 256, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x256x64xbf16>
    %h_input_A = nest.subview %arena offsets = [0] sizes = [65536] strides = [1]
    ...
    %b_A = nest.alloc slot = "A" role = "inout" shape = [4, 256, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x256x64xbf16>
    %h_A = nest.subview %arena offsets = [262144] sizes = [65536] strides = [1]
      : !nest.global_view<65536xbf16>
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_A = nest.alloc slot = "A" role = "in" shape = [4, 256, 64] dtype = "bf16" alignment = 256
      : !nest.l2_buffer<4x256x64xbf16>
    %h_A = nest.subview %arena offsets = [262144] sizes = [65536] strides = [1]
      : !nest.global_view<65536xbf16>
