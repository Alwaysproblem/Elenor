...
builtin.module {
  ...
  tile.program @prog_B (%task: !nest.task, %i0: !nest.l2_buffer<4x256x64xbf16>, %out: !nest.l2_buffer<4x64x64xbf16>) {
    %v0 = tile.subview %i0 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 256, 64] strides = [1, 1, 1] : !nest.l2_view<1x256x64xbf16>
    %l0 = tile.alloc shape = [256, 64] dtype = "bf16" alignment = 256 : !tile.l1_buffer<256x64xbf16>
  ...
  tile.program @prog_C (%task: !nest.task, %i0: !nest.l2_buffer<4x256x64xbf16>, %i1: !nest.l2_buffer<4x64x64xbf16>, %out: !nest.l2_buffer<4x64x64xbf16>) {
    %v0 = tile.subview %i0 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 256, 64] strides = [1, 1, 1] : !nest.l2_view<1x256x64xbf16>
    %l0 = tile.alloc shape = [256, 64] dtype = "bf16" alignment = 256 : !tile.l1_buffer<256x64xbf16>
    %v1 = tile.subview %i1 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 64, 64] strides = [1, 1, 1] : !nest.l2_view<1x64x64xbf16>
    %l1 = tile.alloc shape = [64, 64] dtype = "bf16" alignment = 256 : !tile.l1_buffer<64x64xbf16>
  ...
  tile.program @prog_D (%task: !nest.task, %i0: !nest.l2_buffer<4x256x64xbf16>, %i1: !nest.l2_buffer<4x64x64xbf16>, %i2: !nest.l2_buffer<4x64x64xbf16>, %out: !nest.l2_buffer<4x64x64xbf16>) {
    %v0 = tile.subview %i0 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 256, 64] strides = [1, 1, 1] : !nest.l2_view<1x256x64xbf16>
    %l0 = tile.alloc shape = [256, 64] dtype = "bf16" alignment = 256 : !tile.l1_buffer<256x64xbf16>
    %v1 = tile.subview %i1 task = %task task_dim = 0 offsets = [0, 0, 0] sizes = [1, 64, 64] strides = [1, 1, 1] : !nest.l2_view<1x64x64xbf16>
    %l1 = tile.alloc shape = [64, 64] dtype = "bf16" alignment = 256 : !tile.l1_buffer<64x64xbf16>
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_input_A = nest.alloc slot = "input_A" role = "in" shape = [4, 256, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x256x64xbf16>
    %h_input_A = nest.subview %arena offsets = [0] sizes = [65536] strides = [1] : !nest.global_view<65536xbf16>
    ...
    %b_A = nest.alloc slot = "A" role = "inout" shape = [4, 256, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x256x64xbf16>
    %h_A = nest.subview %arena offsets = [262144] sizes = [65536] strides = [1] : !nest.global_view<65536xbf16>
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_A = nest.alloc slot = "A" role = "in" shape = [4, 256, 64] dtype = "bf16" alignment = 256 : !nest.l2_buffer<4x256x64xbf16>
    %h_A = nest.subview %arena offsets = [262144] sizes = [65536] strides = [1] : !nest.global_view<65536xbf16>
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D tasks(%tasks) globals() bindings(%b_A, %b_B, %b_C, %b_D) ins(%b_A, %b_B, %b_C) outs(%b_D) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A, %pref_B, %pref_C) : (!nest.event<"grid_D">, !nest.event<"read_D">, !nest.event<"ready_D">)
