...
builtin.module {
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_R, %read_R, %ready_R = nest.dispatch.tasks.async @prog_R tasks(%tasks) globals() ins(%b_R0, %b_P2, %b_R) outs(%b_R0, %b_P2, %b_R) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_R0, %pref_P2) : (!nest.event<"grid_R">, !nest.event<"read_R">, !nest.event<"ready_R">)
    nest.release %b_R0 depends_on(%read_R)
    nest.release %b_P2 depends_on(%read_R)
