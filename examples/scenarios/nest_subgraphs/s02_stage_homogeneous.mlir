...
//         "C2": ["C1.output_ready"]
//       },
//       "说明": "全链改为相同 EVU 成本；保留基础版本 weight 的 arena 编号为空洞，确保所有节点输出 tensor index 不变。"
//     }
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_A0, %read_A0, %ready_A0 = nest.dispatch.tasks.async @prog_A0 context = 0 tasks(%tasks)
      globals() bindings(%b_input_A0, %b_A0) ins(%b_input_A0) outs(%b_A0)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_input_A0)
    ...
    %grid_A1, %read_A1, %ready_A1 = nest.dispatch.tasks.async @prog_A1 context = 0 tasks(%tasks)
      globals() bindings(%b_A0, %b_A1) ins(%b_A0) outs(%b_A1)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_A0)
    ...
    %grid_A2, %read_A2, %ready_A2 = nest.dispatch.tasks.async @prog_A2 context = 0 tasks(%tasks)
      globals() bindings(%b_A1, %b_A2) ins(%b_A1) outs(%b_A2)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%ready_A1)
