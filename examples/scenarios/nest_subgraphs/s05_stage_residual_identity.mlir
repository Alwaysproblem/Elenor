...
// case: {"id": "s05", "name": "s05_stage_residual_identity", "nodes": ["X", "F0", "F1", "Add"], "edges": {"F0": ["X"], "F1": ["F0"], "Add": ["X", "F1"]}, "context_partition": [["X"], ["F0", "F1"], ["Add"]], "resource_config": {"uce": 4, "device": 4, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "以实际 service 判定；不把 issue/slot 占用冒充有效重叠"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_X": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_F0": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "X": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "F0": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "F1": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "Add": {"index": 5, "offset_elements": 655360, "bytes": 32768}}, "node_programs": {"X": {"program": "prog_X", "pin": 0, "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "F0": {"program": "prog_F0", "pin": 1, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "F1": {"program": "prog_F1", "pin": 1, "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "Add": {"program": "prog_Add", "pin": 3, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}}, "consumer_ready": {"X": ["input_X.prefetch"], "F0": ["X.context_done 后从同一 HBM 区间 prefetch", "W_F0.prefetch"], "F1": ["F0.output_ready"], "Add": ["X.context_done 后从同一 HBM 区间 prefetch", "F1.context_done 后从同一 HBM 区间 prefetch"]}, "说明": "identity skip 不分配 Copy 输出；Add 真实读取 X 的原 tensor 区间和 F1。"}
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_F0, %read_F0, %ready_F0 = nest.dispatch.tasks.async @prog_F0 context = 1 tasks(%tasks) globals() ins(%b_X, %b_W_F0, %b_F0) outs(%b_X, %b_W_F0, %b_F0) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_X, %pref_W_F0) : (!nest.event<"grid_F0">, !nest.event<"read_F0">, !nest.event<"ready_F0">)
    %grid_F1, %read_F1, %ready_F1 = nest.dispatch.tasks.async @prog_F1 context = 1 tasks(%tasks) globals() ins(%b_F0, %b_F1) outs(%b_F0, %b_F1) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_F0) : (!nest.event<"grid_F1">, !nest.event<"read_F1">, !nest.event<"ready_F1">)
    nest.release %b_X depends_on(%read_F0)
    nest.release %b_W_F0 depends_on(%read_F0)
    %store_F0_0 = nest.dma.store.async %b_F0 into %h_F0 depends_on(%ready_F0, %ready_F1) : !nest.event<"store_F0_0">
    nest.release %b_F0 depends_on(%store_F0_0)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %h_Add = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1] : !nest.global_view<16384xbf16>
    %pref_X = nest.dma.prefetch.async %h_X into %b_X : !nest.event<"pref_X">
    %pref_F1 = nest.dma.prefetch.async %h_F1 into %b_F1 : !nest.event<"pref_F1">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Add, %read_Add, %ready_Add = nest.dispatch.tasks.async @prog_Add context = 3 tasks(%tasks) globals() ins(%b_X, %b_F1, %b_Add) outs(%b_X, %b_F1, %b_Add) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_X, %pref_F1) : (!nest.event<"grid_Add">, !nest.event<"read_Add">, !nest.event<"ready_Add">)
    nest.release %b_X depends_on(%read_Add)
    nest.release %b_F1 depends_on(%read_Add)
