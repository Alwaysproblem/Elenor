...
builtin.module {
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Add, %read_Add, %ready_Add = nest.dispatch.tasks.async @prog_Add context = 3 tasks(%tasks) globals() bindings(%b_X, %b_F1, %b_Add) ins(%b_X, %b_F1) outs(%b_Add) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_X, %pref_F1) : (!nest.event<"grid_Add">, !nest.event<"read_Add">, !nest.event<"ready_Add">)
