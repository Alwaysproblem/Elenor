...
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 3 tasks(%tasks) globals() bindings(%b_A, %b_B, %b_C) ins(%b_A, %b_B) outs(%b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A, %ready_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
