...
// case: {"id": "s08", "name": "s08_single_barrier", "nodes": ["A", "B", "D", "C"], "edges": {"C": ["A", "B"], "D": ["B"]}, "context_partition": [["A", "B", "C", "D"]], "resource_config": {"uce": 4, "device": 1, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "FAIL：刻意在提交 C 和 D 前等待 A/B.ready；工作量、buffer、placement 与正例相同"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_A": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_A": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "input_B": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "A": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "B": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "D": {"index": 5, "offset_elements": 655360, "bytes": 32768}, "C": {"index": 6, "offset_elements": 786432, "bytes": 32768}}, "node_programs": {"A": {"program": "prog_A", "pin": 0, "engine": "boa", "op": "matmul", "repeat": 100, "loads": 1}, "B": {"program": "prog_B", "pin": 1, "engine": "evu", "op": "pow", "repeat": 5, "loads": 1}, "D": {"program": "prog_D", "pin": 2, "engine": "evu", "op": "pow", "repeat": 90, "loads": 1}, "C": {"program": "prog_C", "pin": 3, "engine": "evu", "op": "add", "repeat": 5, "loads": 1}}, "consumer_ready": {"A": ["input_A.prefetch", "W_A.prefetch"], "B": ["input_B.prefetch"], "D": ["B.output_ready"], "C": ["A.output_ready", "B.output_ready"]}, "说明": "C 仍真实读取 A/B；D 仍只依赖 B，不等待 C.ready。反例增加的是 A/B ready barrier。"}
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B context = 1 tasks(%tasks) globals() ins(%b_input_B, %b_B) outs(%b_input_B, %b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_B) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
    nest.await %ready_A, %ready_B
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 3 tasks(%tasks) globals() ins(%b_A, %b_B, %b_C) outs(%b_A, %b_B, %b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A, %ready_B) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D context = 2 tasks(%tasks) globals() ins(%b_B, %b_D) outs(%b_B, %b_D) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_B) : (!nest.event<"grid_D">, !nest.event<"read_D">, !nest.event<"ready_D">)
    nest.release %b_input_A depends_on(%read_A)
    nest.release %b_W_A depends_on(%read_A)
    nest.release %b_input_B depends_on(%read_B)
    %store_A_0 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A, %ready_C) : !nest.event<"store_A_0">
