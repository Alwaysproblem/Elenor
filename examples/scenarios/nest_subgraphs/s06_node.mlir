...
// case: {"id": "s06", "name": "s06_node", "nodes": ["A", "B", "C", "D", "J"], "edges": {"B": ["A"], "C": ["A"], "D": ["C"], "J": ["B", "D"]}, "context_partition": [["A"], ["B"], ["C"], ["D"], ["J"]], "resource_config": {"uce": 4, "device": 4, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "以实际 service 判定；不把 issue/slot 占用冒充有效重叠"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_A": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_B": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "A": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "B": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "C": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "D": {"index": 5, "offset_elements": 655360, "bytes": 32768}, "J": {"index": 6, "offset_elements": 786432, "bytes": 32768}}, "node_programs": {"A": {"program": "prog_A", "pin": "随 device slot", "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "B": {"program": "prog_B", "pin": "随 device slot", "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "C": {"program": "prog_C", "pin": "随 device slot", "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "D": {"program": "prog_D", "pin": "随 device slot", "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "J": {"program": "prog_J", "pin": "随 device slot", "engine": "evu", "op": "add", "repeat": 1, "loads": 1}}, "consumer_ready": {"A": ["input_A.prefetch"], "B": ["A.context_done 后从同一 HBM 区间 prefetch", "W_B.prefetch"], "C": ["A.context_done 后从同一 HBM 区间 prefetch"], "D": ["C.context_done 后从同一 HBM 区间 prefetch"], "J": ["B.context_done 后从同一 HBM 区间 prefetch", "D.context_done 后从同一 HBM 区间 prefetch"]}}
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B tasks(%tasks) globals() bindings(%b_A, %b_W_B, %b_B) ins(%b_A, %b_W_B) outs(%b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A, %pref_W_B) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %pref_A = nest.dma.prefetch.async %h_A into %b_A : !nest.event<"pref_A">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C tasks(%tasks) globals() bindings(%b_A, %b_C) ins(%b_A) outs(%b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_A) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_D, %read_D, %ready_D = nest.dispatch.tasks.async @prog_D tasks(%tasks) globals() bindings(%b_C, %b_D) ins(%b_C) outs(%b_D) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_C) : (!nest.event<"grid_D">, !nest.event<"read_D">, !nest.event<"ready_D">)
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_J, %read_J, %ready_J = nest.dispatch.tasks.async @prog_J tasks(%tasks) globals() bindings(%b_B, %b_D, %b_J) ins(%b_B, %b_D) outs(%b_J) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_B, %pref_D) : (!nest.event<"grid_J">, !nest.event<"read_J">, !nest.event<"ready_J">)
