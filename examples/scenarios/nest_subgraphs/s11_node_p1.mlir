...
// case: {"id": "s11", "name": "s11_node_p1", "nodes": ["P0", "N0", "N1", "N2", "N3"], "edges": {"N0": ["P0"], "N1": ["P0"], "N2": ["P0"], "N3": ["P0"]}, "context_partition": [["P0"], ["N0"], ["N1"], ["N2"], ["N3"]], "resource_config": {"uce": 4, "device": 4, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "以实际 service 判定；不把 issue/slot 占用冒充有效重叠"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_P0": {"index": 0, "offset_elements": 0, "bytes": 32768}, "W_P0": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "P0": {"index": 8, "offset_elements": 1048576, "bytes": 32768}, "N0": {"index": 15, "offset_elements": 1966080, "bytes": 32768}, "N1": {"index": 16, "offset_elements": 2097152, "bytes": 32768}, "N2": {"index": 17, "offset_elements": 2228224, "bytes": 32768}, "N3": {"index": 18, "offset_elements": 2359296, "bytes": 32768}}, "node_programs": {"P0": {"program": "prog_P0", "pin": "随 device slot", "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "N0": {"program": "prog_N0", "pin": "随 device slot", "engine": "copy", "op": "copy", "repeat": 1, "loads": 1}, "N1": {"program": "prog_N1", "pin": "随 device slot", "engine": "copy", "op": "copy", "repeat": 1, "loads": 1}, "N2": {"program": "prog_N2", "pin": "随 device slot", "engine": "copy", "op": "copy", "repeat": 1, "loads": 1}, "N3": {"program": "prog_N3", "pin": "随 device slot", "engine": "copy", "op": "copy", "repeat": 1, "loads": 1}}, "consumer_ready": {"P0": ["input_P0.prefetch", "W_P0.prefetch"], "N0": ["P0.context_done 后从同一 HBM 区间 prefetch"], "N1": ["P0.context_done 后从同一 HBM 区间 prefetch"], "N2": ["P0.context_done 后从同一 HBM 区间 prefetch"], "N3": ["P0.context_done 后从同一 HBM 区间 prefetch"]}, "说明": "1 个 participant；相邻项逐层 reduce，奇数尾项直达下一层，最终真实 Copy fanout 到 N0..N3。"}
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_N0, %read_N0, %ready_N0 = nest.dispatch.tasks.async @prog_N0 tasks(%tasks) globals() ins(%b_P0, %b_N0) outs(%b_P0, %b_N0) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_P0) : (!nest.event<"grid_N0">, !nest.event<"read_N0">, !nest.event<"ready_N0">)
    nest.release %b_P0 depends_on(%read_N0)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %pref_P0 = nest.dma.prefetch.async %h_P0 into %b_P0 : !nest.event<"pref_P0">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_N1, %read_N1, %ready_N1 = nest.dispatch.tasks.async @prog_N1 tasks(%tasks) globals() ins(%b_P0, %b_N1) outs(%b_P0, %b_N1) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_P0) : (!nest.event<"grid_N1">, !nest.event<"read_N1">, !nest.event<"ready_N1">)
    nest.release %b_P0 depends_on(%read_N1)
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %pref_P0 = nest.dma.prefetch.async %h_P0 into %b_P0 : !nest.event<"pref_P0">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_N2, %read_N2, %ready_N2 = nest.dispatch.tasks.async @prog_N2 tasks(%tasks) globals() ins(%b_P0, %b_N2) outs(%b_P0, %b_N2) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_P0) : (!nest.event<"grid_N2">, !nest.event<"read_N2">, !nest.event<"ready_N2">)
    nest.release %b_P0 depends_on(%read_N2)
  ...
  nest.context @ctx_4 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %pref_P0 = nest.dma.prefetch.async %h_P0 into %b_P0 : !nest.event<"pref_P0">
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_N3, %read_N3, %ready_N3 = nest.dispatch.tasks.async @prog_N3 tasks(%tasks) globals() ins(%b_P0, %b_N3) outs(%b_P0, %b_N3) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_P0) : (!nest.event<"grid_N3">, !nest.event<"read_N3">, !nest.event<"ready_N3">)
    nest.release %b_P0 depends_on(%read_N3)
