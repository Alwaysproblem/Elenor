...
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_B, %read_B, %ready_B = nest.dispatch.tasks.async @prog_B context = 1 tasks(%tasks) globals() bindings(%b_A, %b_W_B, %b_B) ins(%b_A, %b_W_B) outs(%b_B) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A, %pref_W_B) : (!nest.event<"grid_B">, !nest.event<"read_B">, !nest.event<"ready_B">)
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 2 tasks(%tasks) globals() bindings(%b_A, %b_C) ins(%b_A) outs(%b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_A) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    ...
    %grid_J, %read_J, %ready_J = nest.dispatch.tasks.async @prog_J context = 3 tasks(%tasks) globals() bindings(%b_B, %b_D, %b_J) ins(%b_B, %b_D) outs(%b_J) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_B, %ready_D) : (!nest.event<"grid_J">, !nest.event<"read_J">, !nest.event<"ready_J">)
    ...
    nest.release %b_B depends_on(%read_J, %store_B_0)
    ...
    nest.release %b_D depends_on(%read_J, %store_D_0)
