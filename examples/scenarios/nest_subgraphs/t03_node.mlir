...
//         "State1->Step1": "producer context_done 后从 State_i HBM 区间 prefetch",
//         "Input1->Step1": "prefetch 完成",
//         "State2->Step2": "producer context_done 后从 State_i HBM 区间 prefetch",
//         "Input2->Step2": "prefetch 完成",
//         "State3->Step3": "producer context_done 后从 State_i HBM 区间 prefetch",
...
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_State1 = nest.alloc slot = "State1" role = "in" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_State1 = nest.subview %arena offsets = [655360] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Step1, %read_Step1, %ready_Step1 = nest.dispatch.tasks.async @prog_Step1 tasks(%tasks)
      globals() bindings(%b_State1, %b_Input1, %b_State2) ins(%b_State1, %b_Input1) outs(%b_State2)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_State1, %pref_Input1)
  ...
  nest.context @ctx_2 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Step2, %read_Step2, %ready_Step2 = nest.dispatch.tasks.async @prog_Step2 tasks(%tasks)
      globals() bindings(%b_State2, %b_Input2, %b_State3) ins(%b_State2, %b_Input2) outs(%b_State3)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
  ...
  nest.context @ctx_3 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    %b_State3 = nest.alloc slot = "State3" role = "in" shape = [4, 64, 64] dtype = "bf16"
      alignment = 256 : !nest.l2_buffer<4x64x64xbf16>
    %h_State3 = nest.subview %arena offsets = [917504] sizes = [16384] strides = [1]
      : !nest.global_view<16384xbf16>
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_Step3, %read_Step3, %ready_Step3 = nest.dispatch.tasks.async @prog_Step3 tasks(%tasks)
      globals() bindings(%b_State3, %b_Input3, %b_State4) ins(%b_State3, %b_Input3) outs(%b_State4)
      signal_policy {
        input_released = #nest.aggregate<all_tasks>
        output_ready = #nest.aggregate<all_tasks>
      } depends_on(%pref_State3, %pref_Input3)
