...
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_C, %read_C, %ready_C = nest.dispatch.tasks.async @prog_C context = 2 tasks(%tasks) globals() ins(%b_input_C, %b_C) outs(%b_input_C, %b_C) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_C) : (!nest.event<"grid_C">, !nest.event<"read_C">, !nest.event<"ready_C">)
    ...
    %store_A_0 = nest.dma.store.async %b_A into %h_A depends_on(%ready_A, %ready_D) : !nest.event<"store_A_0">
