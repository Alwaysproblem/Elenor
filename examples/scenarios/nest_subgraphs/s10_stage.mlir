...
builtin.module {
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_D2, %read_D2, %ready_D2 = nest.dispatch.tasks.async @prog_D2 context = 2 tasks(%tasks) globals() bindings(%b_E2, %b_D2) ins(%b_E2) outs(%b_D2) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_E2) : (!nest.event<"grid_D2">, !nest.event<"read_D2">, !nest.event<"ready_D2">)
