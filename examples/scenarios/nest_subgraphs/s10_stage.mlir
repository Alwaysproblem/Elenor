...
// case: {"id": "s10", "name": "s10_stage", "nodes": ["E0", "E1", "E2", "D2", "D1", "D0"], "edges": {"D2": ["E2"], "D1": ["D2", "E1"], "D0": ["D1", "E0"]}, "context_partition": [["E0", "E1", "E2"], ["D2", "D1", "D0"]], "resource_config": {"uce": 4, "device": 4, "placement": 15, "fidelity": "full_memory"}, "expected": {"时序/生命周期 Correctness": "检查全部真实输入边、完整 Store/release 依赖和 Context 完成", "数值": "未建模；隐式 L1 output timing，仅作合成 compute-cost 扫描", "Liveness": "预期完成", "Scheduling Quality": "decoder Context 只能保守等待整个 encoder Context；不宣称细粒度最优"}, "forbidden_dependencies": "仅允许 edges 与外部输入；跨 Context 使用当前 IR 的保守 context_done 可见性", "tensors": {"input_E0": {"index": 0, "offset_elements": 0, "bytes": 32768}, "input_E1": {"index": 1, "offset_elements": 131072, "bytes": 32768}, "input_E2": {"index": 2, "offset_elements": 262144, "bytes": 32768}, "W_E2": {"index": 3, "offset_elements": 393216, "bytes": 32768}, "E0": {"index": 4, "offset_elements": 524288, "bytes": 32768}, "E1": {"index": 5, "offset_elements": 655360, "bytes": 32768}, "E2": {"index": 6, "offset_elements": 786432, "bytes": 32768}, "D2": {"index": 7, "offset_elements": 917504, "bytes": 32768}, "D1": {"index": 8, "offset_elements": 1048576, "bytes": 32768}, "D0": {"index": 9, "offset_elements": 1179648, "bytes": 32768}}, "node_programs": {"E0": {"program": "prog_E0", "pin": 0, "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "E1": {"program": "prog_E1", "pin": 1, "engine": "evu", "op": "relu", "repeat": 1, "loads": 1}, "E2": {"program": "prog_E2", "pin": 2, "engine": "boa", "op": "matmul", "repeat": 1, "loads": 1}, "D2": {"program": "prog_D2", "pin": 2, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}, "D1": {"program": "prog_D1", "pin": 1, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}, "D0": {"program": "prog_D0", "pin": 0, "engine": "evu", "op": "add", "repeat": 1, "loads": 1}}, "consumer_ready": {"E0": ["input_E0.prefetch"], "E1": ["input_E1.prefetch"], "E2": ["input_E2.prefetch", "W_E2.prefetch"], "D2": ["E2.context_done 后从同一 HBM 区间 prefetch"], "D1": ["D2.output_ready", "E1.context_done 后从同一 HBM 区间 prefetch"], "D0": ["D1.output_ready", "E0.context_done 后从同一 HBM 区间 prefetch"]}}
builtin.module {
  ...
  nest.context @ctx_0 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %grid_E0, %read_E0, %ready_E0 = nest.dispatch.tasks.async @prog_E0 context = 0 tasks(%tasks) globals() ins(%b_input_E0, %b_E0) outs(%b_input_E0, %b_E0) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_E0) : (!nest.event<"grid_E0">, !nest.event<"read_E0">, !nest.event<"ready_E0">)
    %grid_E1, %read_E1, %ready_E1 = nest.dispatch.tasks.async @prog_E1 context = 1 tasks(%tasks) globals() ins(%b_input_E1, %b_E1) outs(%b_input_E1, %b_E1) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_E1) : (!nest.event<"grid_E1">, !nest.event<"read_E1">, !nest.event<"ready_E1">)
    %grid_E2, %read_E2, %ready_E2 = nest.dispatch.tasks.async @prog_E2 context = 2 tasks(%tasks) globals() ins(%b_input_E2, %b_W_E2, %b_E2) outs(%b_input_E2, %b_W_E2, %b_E2) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_input_E2, %pref_W_E2) : (!nest.event<"grid_E2">, !nest.event<"read_E2">, !nest.event<"ready_E2">)
  ...
  nest.context @ctx_1 (%arena: !nest.global_memref<4194304xbf16>) placement = 15 {
    ...
    %tasks = nest.task.range from = 0 to = 4 : !nest.task_range
    %grid_D2, %read_D2, %ready_D2 = nest.dispatch.tasks.async @prog_D2 context = 2 tasks(%tasks) globals() ins(%b_E2, %b_D2) outs(%b_E2, %b_D2) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%pref_E2) : (!nest.event<"grid_D2">, !nest.event<"read_D2">, !nest.event<"ready_D2">)
    %grid_D1, %read_D1, %ready_D1 = nest.dispatch.tasks.async @prog_D1 context = 1 tasks(%tasks) globals() ins(%b_D2, %b_E1, %b_D1) outs(%b_D2, %b_E1, %b_D1) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_D2, %pref_E1) : (!nest.event<"grid_D1">, !nest.event<"read_D1">, !nest.event<"ready_D1">)
    %grid_D0, %read_D0, %ready_D0 = nest.dispatch.tasks.async @prog_D0 context = 0 tasks(%tasks) globals() ins(%b_D1, %b_E0, %b_D0) outs(%b_D1, %b_E0, %b_D0) signal_policy { input_released = #nest.aggregate<all_tasks>, output_ready = #nest.aggregate<all_tasks> } depends_on(%ready_D1, %pref_E0) : (!nest.event<"grid_D0">, !nest.event<"read_D0">, !nest.event<"ready_D0">)
    nest.release %b_E2 depends_on(%read_D2)
    nest.release %b_E1 depends_on(%read_D1)
    nest.release %b_E0 depends_on(%read_D0)
    %store_D2_0 = nest.dma.store.async %b_D2 into %h_D2 depends_on(%ready_D2, %ready_D1) : !nest.event<"store_D2_0">
    nest.release %b_D2 depends_on(%store_D2_0)
    %store_D1_0 = nest.dma.store.async %b_D1 into %h_D1 depends_on(%ready_D1, %ready_D0) : !nest.event<"store_D1_0">
    ...
    nest.await %grid_D2, %grid_D1, %grid_D0, %store_D2_0, %store_D1_0, %store_D0_0
    nest.return
  }
