...
分析工具链（2026-09-13 起）改为 **Perfetto trace_processor（v58.2）SQL**：查询文件 [perfetto_queries.sql](../../artifacts/nest_subgraphs/sec9_ready_action_audit/perfetto_queries.sql) / [perfetto_queries_release.sql](../../artifacts/nest_subgraphs/sec9_ready_action_audit/perfetto_queries_release.sql)，结果 [metrics_perfetto.json](../../artifacts/nest_subgraphs/sec9_ready_action_audit/metrics_perfetto.json)，并与独立 Python 提取器逐字段比对为零差异。单位换算：tracer（`trace.py:189-192`）把 cycle 转成**微秒**写入 Chrome JSON，trace_processor 再把 µs 归一化为 ns；故本审计在 `cycle_ns()==1`（默认 1000MHz）下 **raw `ts/dur` 原值即 cycles**。通用规则为 `cycles = ts_ns / cycle_ns`，换主频时需按此换算，不可照搬本目录数字。

**parse 完整性审计**（[perfetto_parse_audit.json](../../artifacts/nest_subgraphs/sec9_ready_action_audit/perfetto_parse_audit.json)）：trace_processor 对 9 份 trace 报告 101–321 条 `json_parser_failure`。逐类核对结论：本分析依赖的全部类别（X 计算切片、dispatch 切片、phase_aggregate/l2_release instant、全部 counter）**逐一与原始 JSON 计数精确相等**，slice 总数与 X+i+B/E 配对数精确吻合（如 5331=1000+4267+64），因此 **§9.3 全部指标基于完整数据**。剩余损耗从 raw ph 计数与 `flow` 表对比观测为 legacy flow 事件绑定残差（约每份 11 条 flow 未能入表），但未做 event 级逐一溯源；该残差不参与任何指标计算，仅可能影响 UI 的流箭头呈现。
