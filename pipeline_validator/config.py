  ...
  def walk(node: dict[Any, Any], dotted: str) -> None:
    for key, value in node.items():
      if not isinstance(key, str):
        raise ValueError(f"non-string key {key!r} under '{dotted or '<root>'}' in '{path}'")
...
def _load_hw_yaml(path: Path, *, required: bool) -> dict[str, Any]:
  ...
  if not isinstance(raw, dict):
    raise ValueError(f"hardware config '{path}' must be a mapping, got {type(raw).__name__}")
  ...
  if required:
    ...
    if missing:
      raise ValueError(f"missing required HardwareConfig fields in '{path}': {missing}")
  return flat
...
class HardwareConfig:
  ...
  def __post_init__(self) -> None:
    ...
    if self.hbm_burst_bytes <= 0 or (self.hbm_burst_bytes & (self.hbm_burst_bytes - 1)) != 0:
...
@dataclass(frozen=True)
class DeviceConfig:
  """CPU interpreter limits, independent of FPGA Group/Tile resources."""

  pending_capacity: int = 32
  completion_capacity: int = 4096
  issue_width: int = 1
  ...
  def __post_init__(self) -> None:
    for item in fields(self):
      ...
      if not isinstance(value, int) or isinstance(value, bool) or value < 1:
        raise ValueError(f"device.{item.name} must be a positive integer")


@dataclass(frozen=True)
class GroupSchedulerConfig:
  """Finite hardware-model resources; values are exploration parameters."""
  ...
  def __post_init__(self) -> None:
    if self.policy not in ("s0", "s1", "s2"):
      raise ValueError("group.policy must be s0, s1 or s2")
    if self.epoch_policy not in ("mixed", "same_program"):
      raise ValueError("group.epoch_policy must be mixed or same_program")
    for item in fields(self):
      if item.name in ("policy", "epoch_policy"):
        continue
      value = getattr(self, item.name)
      if not isinstance(value, int) or isinstance(value, bool) or value < 1:
        raise ValueError(f"group.{item.name} must be a positive integer")
    if self.context_action_quota > self.action_capacity:
      raise ValueError("group.context_action_quota must not exceed action_capacity")
    if self.scan_width > self.action_capacity:
      raise ValueError("group.scan_width must not exceed action_capacity")
...
class SimConfig:
  ...
  group: GroupSchedulerConfig = field(default_factory=GroupSchedulerConfig)

  def __post_init__(self) -> None:
    if not isinstance(self.device, DeviceConfig) or not isinstance(self.group, GroupSchedulerConfig):
      raise ValueError("device and group must be their respective configuration objects")
    if self.fidelity not in ("timing_only", "runtime", "full_memory"):
      raise ValueError("fidelity must be one of: timing_only, runtime, full_memory")
  ...
  def with_overrides(self, **kw) -> SimConfig:
    for scope in ("device", "group"):
      scoped = {key[len(scope) + 1 :]: kw.pop(key) for key in tuple(kw) if key.startswith(scope + ".")}
      if scoped:
        kw[scope] = replace(getattr(self, scope), **scoped)
    return replace(self, **kw)
