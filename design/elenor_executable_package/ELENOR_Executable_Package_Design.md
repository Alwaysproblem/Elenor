...
- 固化 graph schedule、region program、tile program、descriptor table、relocation table、weight section 和 runtime metadata 的组织方式。
...
- 保证硬件只执行 command、descriptor、Region Program 和 Tile Program；program residency 由 hardware/firmware 隐式保证，而不是 package 中的显式 load 指令。
...
| Program           | Region Program 与 Tile Program 分 section 存储，program_id 稳定   | 多版本 program cache policy                      |
...
1. 描述执行对象之间的静态关系：Graph Schedule -> Pipeline Region -> Region Program -> Tile Program -> Descriptor Template。
...
| Region Program      | Compiler + firmware kernel library      | Firmware              | residency metadata                     | Region Sequencer    |
...
- `ResidentReady`：hot Region Program 已可被 Region Sequencer fetch，hot Tile Program 已可被 Tile UCE fetch 或可按需从 HBM 装入。
...
| Group SRAM  | hot Region Program                           | Region Sequencer / firmware | HBM -> Group SRAM load      |
...
- 列出每个 region 的 expected PMU fingerprint，例如 BOA active、MFE prefetch miss、stream credit stall。
- 标记 command_id、region_id、program_id、descriptor_id 到 counter group 的映射。
...
T_region = max(T_group_dma, T_tile_compute_pipeline, T_stream_backpressure)
...
- Runtime dialect 产生 command template、event dependency、region launch。
- Package dialect 或 packer IR 只描述 section、relocation、manifest，不混入 engine 语义。
...
elenor.package @llm_block target(#elenor.profile<balanced>) {
  elenor.entry @decode_step(%q, %kv_pages, %out) {
    %region = elenor.region @paged_attention
    elenor.command_template launch_region %region
      wait_events = []
