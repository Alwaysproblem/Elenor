...
该 overlap 只有在 buffer independence、async queue credit 和 event sequence 均成立时有效；否则应计入 `uce_slot_hazard_stall`、`mfe_stream_credit_full` 或 `mfe_store_visibility_wait`，不能把 unsafe overlap 记入性能收益。V1.x 验证模型中，queued next TileProgram 的 leading MFE load 前缀可在 active program 仍运行时提前发射到 MFE（restricted MFE-load-prefix lookahead，见 `elenor_tile_uce` §3.5.1），MFE 只接受 `kind==MFE && op==load` 的 lookahead launch，store 和 compute launch 不进入 lookahead；MFE busy 时 lookahead retry 而不阻塞 active UCE。
...
- UCE V1.x sliding window trace 中，P0 store 与 P1 load overlap 只有在 slot 独立时出现；store visibility event 和 PMU wait 指纹可解释。V1.x 验证模型的 restricted MFE-load-prefix lookahead（见 `elenor_tile_uce` §3.5.1）要求 MFE 只接受 `kind==MFE && op==load` 的 namespaced lookahead launch，trace 中 `lookahead:` event id 可作为 implementation-private 证据；store/compute launch 不进入 lookahead，MFE busy 时 lookahead retry 而不阻塞 active UCE。
