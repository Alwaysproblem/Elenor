...
ELENOR PMU 是 Architecture V1 中连接硬件执行、runtime 调度、compiler 性能模型和 bring-up 验收的观测面。硬件执行 command、descriptor、TileGroupTask 和 Tile Program；PMU 不解释高层 graph，只记录这些执行对象在 Chip、Tile Group、Compute Tile 和 engine 层面的周期、带宽、队列、事件、错误与 stall 归因。
...
- Group PMU 聚合 Tile Group Sequencer、Group DMA、Stream Queue、Collective、Shared SRAM/L2 和 group barrier。
...
- command boundary snapshot 用于单 command profiling；group task boundary snapshot 用于 role dispatch profiling；global periodic snapshot 用于 QoS 和 thermal/power 分析。
...
- runtime 在 command begin/end 或 group task begin/end 发出 PMU marker。
