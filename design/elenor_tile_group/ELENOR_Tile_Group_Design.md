...
Tile Group 是 ELENOR 的局部数据复用、局部同步和 group 内 pipeline 执行域。它位于 Device Runtime 与 Compute Tile 之间，承接 Pipeline Region，将 HBM/DDR/LPDDR 经 Group DMA 预取到 Group Shared SRAM/L2，再通过 Tile DMA、Stream Queue、Barrier/Event 和 Collective Engine 组织多个 Compute Tile 协同执行。

核心目标：

- 降低所有 tile 直接访问全局内存和全局 NoC 的压力。
- 在 group 内提供可验证的 stage pipeline：block 粒度流动，而不是 stage 全量串行。
- 将 program control、data movement、synchronization、collective 和 PMU attribution 分层，避免 Region Sequencer、Tile UCE、MFE、USE 职责重叠。
...
| ------------- | ----------------------------------------------------------------------- | --------------------------------------------- |
| Region 执行   | 单 group region task、固定 stage graph、有限分支                        | priority/preemption、多 context QoS           |
...
| Barrier/Event | group barrier、stage event、tile done、group done、timeout              | 分层 global barrier、采样 trace               |
...
- Region Sequencer：维护 Region PC，推进 Device Pipeline，dispatch stage，管理 group barrier/event。
...
- Stream Queue Engine：stage 间 token、credit、backpressure、EOS/error 传播。
- Barrier/Event Engine：group 内 tile 同步、DMA completion、stage synchronization、fault propagation。
...
1. Region Sequencer owns program control；hardware 执行 Region Program，不执行高层 graph。
2. Group DMA owns HBM 到 L2 的显式 copy；Tile DMA owns L2 到 L1；二者 completion event 必须区分 producer_id。
3. Stream Queue owns token 生命周期和 credit；Barrier/Event owns event 状态机；不能由 stage program 直接篡改队列内部 credit。
4. Shared SRAM/L2 的 allocation 由 descriptor/region frame 显式声明；DMA、Collective、Stream Queue、Tile Dispatcher 的写 owner 必须唯一。
...
- control path：Region Sequencer -> Tile Dispatcher / DMA / Queue / Barrier / Collective。
- event path：所有 engine -> Barrier/Event Engine -> Region Sequencer / Runtime event table。
...
- PMU path：各 engine 输出 active/stall/event，Group PMU 聚合并按 context_id、region_id、stage_id、engine_id 标记。
...
- `ACCEPT_TASK`：锁存 context_id、region_id、descriptor pointer、wait/signal event。
...
- `PROGRAM_FETCH_ON_MISS`：cold launch 时由 Group DMA 将 region program 搬入 Group Program Cache；同一 key 已在抓取时只挂等待，不重复发请求。
...
- `RUN`：Region Sequencer 只在 `program_ready`、descriptor window ready 和依赖满足后按 Region PC issue 指令，stage 以 block token 流动。
...
Pipeline Region 是 stage graph，不是高层 graph。stage 之间以 block token 连接：

```text
Stage0(preload/produce) -> S0 -> Stage1(compute) -> S1 -> Stage2(store/merge)
```

必须满足：

1. Stage0 不需要完成全部 block 后 Stage1 才开始；只要 S0 有 valid token 且 Stage1 有 credit/资源即可并行。
...
5. error token 携带 fault_record_slot，并沿 stream 传播到 region completion。
...
| group reset  | Region Sequencer、Group DMA、queue、barrier、collective、L2 window | drain 或 kill in-flight，清 queue credit，poison 未完成 event             |
...
} elenor_group_region_launch_desc_v1_t;
```

未冻结字段的 alignment、endianness、CRC policy、flag bit 编码由后续规格冻结。

### 4.2 Region resource descriptor
...
typedef struct {
    ...
    uint32_t flags;
} elenor_group_region_resource_desc_t;
```

协议要求：

- `l2_window_base/bytes` 必须落在 group SRAM allocation 内，且不覆盖 active resident program、descriptor、event 区。
- `queue_count` 对应 Stream Queue descriptor table；每个 queue 的 producer/consumer mask 必须与 stage graph 一致。
...
- descriptor patch 只允许由 Runtime/Region Sequencer 在 region start 前完成，RUN 状态下修改必须通过显式 fence。
...
| `REGION_PC`             | 当前 Region PC 或 fault PC                         |
| `ACTIVE_CONTEXT_REGION` | context_id、region_id、group_id                    |
...
### 4.4 Region Program command examples
...
region.begin r0
    ...
    dispatch.stage stage=0, tile_mask=0x0f, program=tile_kernel_qk, out=s0
    dispatch.stage stage=1, tile_mask=0xf0, program=tile_kernel_softmax, in=s0, out=s1

loop_blocks:
    wait.credit   s0
    dma.prefetch  desc=kv_next, dst=l2_kv_next -> ev_dma1
    signal.stage  stage=0, block=block_id
    wait.stream   s0
    signal.stage  stage=1, block=block_id
    advance.block
...
- Region Sequencer issues Group DMA descriptors for HBM 到 L2 prefetch/storeback。
...
- Group DMA completion event 可作为 stage token 生产条件，也可作为 Region Sequencer wait 条件。

### 5.2 控制流

```text
Device Runtime command queue
  -> ELENOR_CMD_LAUNCH_REGION
  -> Region Sequencer Region PC
  -> Tile Dispatcher / Group DMA / Stream Queue / Barrier / Collective
  -> Tile UCE tile task
  -> engine completion event
  -> stage event / group done event
  -> Runtime event table
```
...
- Region Sequencer 的 wait 指令必须带 timeout 或受 region timeout 覆盖。
...
- Region PC decode -> issue arbitration -> engine command valid。
- Queue push/pop -> occupancy/credit update -> backpressure ready。
- Barrier arrived mask reduction -> release event。
- DMA completion -> event scoreboard -> Region wait wakeup。
...
| 分区                  | 用途                                         | owner                          |
| --------------------- | -------------------------------------------- | ------------------------------ |
| region program/cache  | Region Program、descriptor window            | Region Sequencer               |
| stream buffer         | token payload、stage ring buffer             | Stream Queue / producing stage |
...
- `group_active_cycles`。
- `region_issue_cycles`、`region_wait_event_cycles`、`region_wait_stream_cycles`。
...
- Region Sequencer 使用小型 fixed-width decoder + scoreboard，不实现通用 CPU。
- engine issue 采用 ready/valid，所有 command 带 context_id、region_id、stage_id、sequence。
- Stream Queue head/tail/credit 使用显式 invariant 检查；credit counter 宽度由 depth 推导，不手写 magic width。
- Barrier/Event Engine 与 Region Sequencer 分离，避免 wait wakeup 和 event write 形成组合环。
...
- compiler 生成 group sequence、stage graph、stream queue descriptor，以及 `program_id/template_id/program_ref/cache hint`。
...
- Error propagation：任意 producer error token 最终导致 region event ERROR 或按 descriptor policy 转换为可恢复状态。
...
2. launch empty region，signal group done event。
...
5. two-stage pipeline overlap，观察 Stage1 在 Stage0 未完成全部 block 前启动。
...
- Region Sequencer 过度通用化：若演化成小 CPU，会增加验证和时序风险。
...
- Region Program ISA 保持控制面指令；数据相关动态访问交给 MFE，tile-local kernel pipeline 交给 Tile UCE。
- Group SRAM 使用 descriptor window 和 placement hint，不承诺 cache coherency。

后续需要冻结：

- Group SRAM profile、bank/port/ECC、arbiter policy。
- Region Program binary encoding、CSR map、fault code。
