...
- RegionLaunchDesc patch/fanout、program residency contract orchestration、reset/drain epoch 管理。
...
9. firmware dispatches region program
10. Region Sequencer dispatches tile programs
...
- long-running region 使用 interrupt 或 hybrid polling。
...
- Region scheduling 以 pipeline region 为粒度，不逐 tile 做全局调度。
...
| 风险                     | 影响                   | 缓解                                                      |
| ------------------------ | ---------------------- | --------------------------------------------------------- |
| Firmware 过度复杂        | 变成小 OS，验证困难    | 只消费 command/descriptor/program，region 粒度调度        |
