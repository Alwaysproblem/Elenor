...
Stream Queue 是 ELENOR Pipeline Region 中 stage 间的硬件 producer-consumer contract。它连接 Region Sequencer、Tile UCE、MFE、DMA、BOA/EVU/USE 后续 stage 和 event fabric，使 block 粒度数据可以在 stage 间流动，而不是等待上游 stage 全量完成。

Stream Queue 不计算数据，不解释高层 graph，不替代 event/barrier。它只管理 token、payload 引用、credit、backpressure、EOS、error propagation 和 reset/drain。硬件执行的是 Region Program / Tile Program 中的 stream 指令与 stream descriptor。
...
| error       | error token 携带 fault record index 并传播到 region/tile completion               | retryable error、partial replay                       |
...
| queue init / reset / drain               | Region Sequencer                | Region 进入、退出、fault recovery 时执行                   |
...
- 不做 graph scheduling；Region Sequencer 选择 stage，Stream Queue 只表达 backpressure。
- 不隐式吞掉错误；error token 必须可被 consumer、Region Sequencer 和 fault record 观测。
...
指令由 Tile UCE 或 Region Sequencer 执行。UCE 拥有 Tile Program PC、branch、engine launch 和 descriptor patch；USE 只在 state/recurrence 路径需要时消费或产生状态 token，不拥有主控制流。
...
### 5.1 Region pipeline 数据流

```text
Region Sequencer:
  INIT_STREAM S0
  DISPATCH_STAGE producer_tiles
  DISPATCH_STAGE consumer_tiles
...
- error token 不丢弃已可见的早期 valid token；Region policy 可选择 stop affected queue 并 drain。
...
- 环形依赖：compiler/runtime 不能构造所有 stage 同时等待下游 credit 的不可打破循环；硬件需提供 watchdog counter 辅助定位。

### 5.4 EOS/error/reset 传播

| token       | 行为                                                                      | completion 影响                 |
| ----------- | ------------------------------------------------------------------------- | ------------------------------- |
| VALID       | consumer 处理 payload，release 后 credit 回收                             | 不直接完成 region               |
| EOS         | consumer 标记 producer done；all-EOS 时 stage done                        | 可触发 downstream EOS           |
| ERROR       | consumer 停止普通处理，记录 fault，向 downstream 推 error 或 region fault | region/tile completion 带 fault |
...
| Balanced |           32 到 64 |     3 到 8 | 由 SRAM profile 冻结 | paged attention / MoE stage overlap |
| High End |          64 到 128 |    4 到 16 | 由 SRAM profile 冻结 | 多 pipeline、多 context             |

First Silicon 可从 depth=3 的 canonical trace 开始，因为源文档 Region Program 示例使用 `depth=3`；产品值由后续规格冻结。

### 6.2 性能模型

```text
T_stage_pipeline = max(T_producer, T_consumer, T_memory) + T_queue_overhead
...
- Queue descriptor shadow 在 Region init 时一次加载，运行中只允许状态寄存器变化。
...
- producer_id、consumer_id 从 Tile UCE / Region Sequencer 的硬件 ID 派生，避免软件伪造越权 producer。
...
- EOS all-producer：all-EOS policy 下缺少任一 producer EOS 不得 stage done。
...
| error token injection | fault propagation            | region completion 带 fault record              |
...
Bring-up 顺序：先 SPSC FIFO formal，再 RTL random push/pop，再接 Tile UCE stream 指令，再接 Region Program pipeline，最后接 MFE Page Stream EOS/error。
...
- Patch ownership：stream descriptor 静态字段由 Compiler，context/base/residency 由 Runtime/firmware，queue init/drain 由 Region Sequencer，token 操作由 Tile UCE 或 engine wrapper。
...
| EOS 语义含混                  | stage 过早结束或无法结束       | descriptor 中显式 EOS policy 和 producer mask             |
| multi-consumer 过早实现       | refcount/reset 验证爆炸        | First Silicon 使用独立 queue 表达广播                     |
| error token 被普通 token 淹没 | fault 延迟不可控               | error latch + VC0 fault fabric + Region policy stop queue |
