...
7. Stream Queue 的 token/credit/EOS/error 内部状态由 Stream Queue Engine 拥有；Tile Group Sequencer 只 init/reset/drain 并通过 Event/Fault 观察。
...
| `collective.run`      | issue group collective                     | collective_desc_id、event_id、signal_sequence         |
| `signal.event`        | signal group/tile event                    | event_id、sequence、status                            |
| `group_task.complete` | drain 并完成 group task                    | signal_event、signal_sequence                         |
...
group_task.accept  task=attention_task
    ...
    dma.prefetch  desc=input_qk, dst=l2_buf0 -> ev_dma0:seq_dma0
    wait.event    ev_dma0 seq=seq_dma0

    dispatch.role role_id=0 event=ev_role0 seq=seq_role0  # QK producer, tile_mask=0x03, out_stream=0
    dispatch.role role_id=1 event=ev_role1 seq=seq_role1  # softmax+AV consumer, tile_mask=0x0C, in_stream=0
    wait.event    ev_role1 seq=seq_role1

    dma.store     desc=output_av, src=l2_out -> ev_dma_out:seq_dma_out
    wait.event    ev_dma_out seq=seq_dma_out
...
- `wait_ref_iova` / `wait_ref_count` / `wait_ref_crc_or_zero` 指向 `(event_id, expected_sequence)` 等待集合；`signal_event` / `signal_sequence` 定义 group task completion event。
...
- 若启用 V1.x dual-context prepared tile dispatch，admission miss 只表现为“本次未 dispatch 到 UCE”，不会伪造 tile done；一旦 prepared task 被 UCE accept，completion/fault 仍按 tile terminal path 可见。
...
- Dual-context admission ownership 漂移：若把 eligibility/FIFO/next-program selection 下推到 UCE，会重新引入 group-level program semantics。
...
- 若后续启用 dual-context，也只扩 prepared tile dispatch admission，不新增 `Tile Group Program IR` 或 UCE 内 central program FIFO。
