...
- Device Runtime、Region Sequencer、Tile UCE、Stream Queue 的控制流模型。
...
+ Region/Tile program execution model
```
...
ELENOR 的硬件消费 descriptor、command buffer、Region Program 和 Tile Program，不直接消费高层 graph。高层动态图由 compiler/runtime 降到可执行 command sequence、descriptor table 和 executable package。

一句话总结 ELENOR 的执行模型：

```text
Graph Schedule 描述 region 依赖；
Region Program 推进 device pipeline；
Tile Program 推进 tile-local kernel pipeline；
Stream Queue 连接 stage；
...
```text
ELENOR 硬件执行 descriptor、command buffer、Region Program 和 Tile Program；
...
- Device Runtime 不逐 tile 调度细节，而调度 Pipeline Region。
...
| Global Scheduler  | 解析 graph schedule，调度 Pipeline Region，管理全局 event 和资源。                                            |
...
- Group 内数据复用。
- Group 内 pipeline stage 管理。
- HBM/DDR/LPDDR -> L2 预取。
- Stage 间 producer-consumer stream。
...
| 模块                       | 层级       | 核心职责                                                                             | 不负责                               |
| -------------------------- | ---------- | ------------------------------------------------------------------------------------ | ------------------------------------ |
| Region Sequencer           | Tile Group | Region PC 推进、Device Pipeline 控制、Stage Dispatch、Region 级同步                  | BOA 执行、Vector 执行、Tile 内部调度 |
| Stream Queue Engine        | Tile Group | Producer-Consumer 队列、Backpressure、Credit 管理、EOS 传播                          | 数据计算                             |
| Barrier / Event Engine     | Tile Group | Tile 同步、Stage 同步、DMA 完成通知、事件传播                                        | 数据搬运                             |
...
| Tile Group   | group 内同步、broadcast、collective、shared SRAM、Region Sequencer      | 全芯片 memory policy                 |
| Chip Runtime | command buffer、event、shape dispatch、multi-model scheduling           | 高层 graph 解释                      |

### 7.1 控制流层次

```text
Graph Schedule PC / Region Iterator
        |
        v
Region PC
        |
...
| 控制层        | 控制器              | 管理对象                                                    |
| ------------- | ------------------- | ----------------------------------------------------------- |
| Graph Level   | Device Runtime      | Graph schedule、region dependency、context、queue           |
| Region Level  | Region Sequencer    | Device Pipeline、Group DMA、stage dispatch、group barrier   |
...
| Stage 到 Stage                  | Stream Queue Engine                |
...
| Pipeline 层      | 推进者              | 粒度                           |
| ---------------- | ------------------- | ------------------------------ |
| Device Pipeline  | Region Sequencer    | Stage / Region                 |
...
| Group Scheduler / Region Sequencer | group 内 region dispatch、barrier、collective 协调、HBM 到 L2 预取                     |
...
| per region       | stage 间缓存或 partial result               | Region Sequencer / Stream Queue |
...
Program 不是整个 graph，也不是单个 op，而是 Pipeline Region / Kernel Tile Program。
...
Tile Group Region Sequencer:
    receive region task
    lookup Program Residency Manager
    fetch/verify/install region program on miss
    init stream queues
    dispatch prepared stages
...
## 16. Region Program、Tile Program 和 Stream Queue Contract

### 16.1 Device Pipeline 语义

一个 Pipeline Region 可以有多个 stage：
...
```text
不是：Stage0 全部完成 -> Stage1 开始 -> Stage2 开始
而是：Block 粒度流动，stage 之间通过 stream queue 连接。
```
...
4. **error token**：error token 必须携带 fault record index，并沿 stream 传播到 region/tile completion。
...
### 16.3 Region Program 示例

Region Program 运行在 Tile Group 的 Region Sequencer 上。

```asm
REGION_BEGIN

    INIT_STREAM     S0, depth=3
    INIT_STREAM     S1, depth=3

    DMA_PREFETCH    block=0, dst=L2_BUF0
    DMA_PREFETCH    block=1, dst=L2_BUF1

    DISPATCH_STAGE  stage0, tile_mask=0x0f, program=tile_kernel_a
    DISPATCH_STAGE  stage1, tile_mask=0xf0, program=tile_kernel_b

loop_blocks:
    WAIT_CREDIT     S0

    DMA_PREFETCH    next_block, dst=L2_NEXT

    SIGNAL_STAGE    stage0, current_block
    WAIT_STREAM     S0

    SIGNAL_STAGE    stage1, current_block
...
Region Sequencer 的职责：

1. 维护 Region PC。
2. 初始化 stream queue。
3. 控制 Group DMA HBM 到 L2。
4. Dispatch tile stage。
5. 处理 stage/event/barrier。
6. 推进 pipeline region。
...
| grid-level sync     | group barrier / region barrier    |
...
```mlir
region.pipeline @region0(%input, %output) {
  __omp_magic("s0", "= region.stream depth = 3 : !region.stream")

  region.prefetch %input[%c0] to %l2_buf0
  region.prefetch %input[%c1] to %l2_buf1

  region.dispatch @stage0
      tile_mask = 0x0f
      program = @tile_kernel0
      out_stream = __omp_magic("s0", "")

  region.dispatch @stage1
      tile_mask = 0xf0
...
8. Region Program ISA 和 Tile UCE ISA 的编码格式。
...
- program control 交给 Tile UCE / Region Sequencer / Runtime。
- stateful compute 交给 USE。

Device Runtime 调度 Pipeline Region；Region Sequencer 推进 Device Pipeline；Tile UCE 推进 tile-local kernel pipeline；USE 管理 state/scan/recurrence；Stream Queue 连接 stage；Group DMA 搬 HBM -> L2；Tile DMA 搬 L2 -> L1；BOA/EVU/MFE/USE 完成实际计算、数据流和状态更新。
